...
/*===== business rule definition =====
{
  ...
  "type" : "BusinessAction",
  "setupGroups" : [ "Actions" ],
  "name" : "Shot Request - Add Shared Market WebUI(2)",
  "description" : null,
  "scope" : "Global",
  "validObjectTypes" : [ ],
...
function addSharedMarkets(shotRequest) {
    ...
    log.info(" Shot Request Status is: " + shotRequestStatus);
    ...
    if (shotRequestStatus != null) {
        ...
        log.info("SHAREDMARKETS" + sharedMarkets);
    ...
    if (shotRequestStatus == "Draft") {
        var currentSharedMarkets = shotRequest.getValue("a_Shared_Markets").getSimpleValue();
        if (currentSharedMarkets.contains("US")) {
            shotRequest.getValue("a_Shot_Primary_Market").setSimpleValue("US");
        }
        else if (currentSharedMarkets.contains("CAN")) {
            shotRequest.getValue("a_Shot_Primary_Market").setSimpleValue("CAN");
        }
        else if (currentSharedMarkets.contains("JPN")) {
            shotRequest.getValue("a_Shot_Primary_Market").setSimpleValue("JPN");
        }
        else if (currentSharedMarkets.contains("SA")) {
            shotRequest.getValue("a_Shot_Primary_Market").setSimpleValue("SA");
        }
        shotPrimaryMarket = shotRequest.getValue("a_Shot_Primary_Market").getSimpleValue();
