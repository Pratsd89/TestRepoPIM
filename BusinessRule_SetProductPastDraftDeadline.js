...
{
  "id" : "SetProductPastDraftDeadline",
  "type" : "BusinessAction",
  "setupGroups" : [ "Actions" ],
  "name" : "SetProductPastDraftDeadline",
  "description" : null,
  "scope" : "Global",
  "validObjectTypes" : [ ],
...
deadlineTime = node.getTaskByID("wf_NewStyleEnrichment", "NewStyleEnrichState1").getDeadline().getTime();
...
if(parseInt(deadlineTime) < parseInt(currTime)){
	...
	deadlineTime = node.getTaskByID("wf_CCEnrichment", "NewCCEnrichState1").getDeadline().getTime();
	...
	if(parseInt(deadlineTime) < parseInt(currTime)){
		...
		deadlineTime = node.getTaskByID("wf_NewSKUEnrichment", "NewSKUEnrich1").getDeadline().getTime();
