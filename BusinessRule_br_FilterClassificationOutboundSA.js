...
{
  "id" : "br_FilterClassificationOutboundSA",
  "type" : "BusinessCondition",
  "setupGroups" : [ "Conditions" ],
  "name" : "Filter Classification Outbound Condition SA",
  "description" : null,
  "scope" : "Global",
  "validObjectTypes" : [ ],
...
if (obj == "WebCategory" || obj == "WebSubCategory" || obj=="WebDivision") {
        ...
        var saCurrentClass = saContextManager.getClassificationHome().getClassificationByID(node.getID());
        logger.info("Retrieved Classification for Node ID: " + node.getID());

        if (saCurrentClass == null) {
            logger.info("Error: saCurrentClass is null for Node ID: " + node.getID());
            throw "saCurrentClass is null. Node ID is: " + node.getID();
        }

        var objType = saCurrentClass.getObjectType().getID();
        logger.info("Classification Object Type: " + objType);

        // Retrieve values for a_Category_Description and a_WebCategory_Start_Date
        var catDesc = saCurrentClass.getValue("a_Category_Description").getSimpleValue();
        var webStartDate = saCurrentClass.getValue("a_WebCategory_Start_Date").getSimpleValue();
        ...
        if (catDesc != null && webStartDate != null) {
            ...
            if (objType == "WebCategory" || objType == "WebSubCategory") {
                var parentObject = saCurrentClass.getParent();
                if (parentObject) {
