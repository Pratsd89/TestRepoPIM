...
{
  "id" : "testInherit_webui",
  "type" : "BusinessAction",
  "setupGroups" : [ "TestBRs_Indu" ],
  "name" : "Test Inherit US Copy WebUI Actions",
  "description" : null,
  "scope" : "Global",
		...
		if (inheritCopyMkts != null) {
			inheritCopyMkts.forEach(function (mkt) {
				if (mktDesignation.contains(mkt.getValue()) == true) {
					...
					mkt.deleteCurrent();
					webUI.showAlert("WARNING:", null, "Inherit US Copy Option of " + mkt + " was removed, as this is not a valid market for the Style");
				}
			});
		}
