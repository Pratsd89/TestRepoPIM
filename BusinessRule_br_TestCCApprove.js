...
*/
exports.operation0 = function (step,CCToShotRequestRef,shotRequestToExternalAssetRef,node,LKT,compCheck) {
/*
...
//if (!node.isInState(wfID, "NewCCEnrich_Photo3") && !node.isInState(wfID, "NewCCEnrich_Final")) //{
    //webUI.showAlert("WARNING",  "This CC " + node.getName() + " and Photos are not eligible for approval at this time");
//}
//else {
    var shotRequestReferences = node.getReferences(CCToShotRequestRef).toArray();
    var imageCheck = compCheck.checkCCAssetCompleteness(node, step);

    //If image check is NOT true
    if (imageCheck != true) {
        //webUI.showAlert("WARNING",  imageCheck);
    }
    else {
        ...
        for (var i = 0; i < shotRequestReferences.length; i++) {
            ...
            if (shotRequest.isInState("wf_ShortRequestLifeCycle", "Ready_to_Review")) {
                ...
                try {
                	BusinessRuleHome.getBusinessActionByID("wf_ShortRequestLifeCycle_Ready_to_Review").execute(shotRequest);
        ...
        if (node.isInState(wfID, "NewCCEnrich_Photo3")) {
            ...
            if (wfSubmit.isRejectedByScript()) {
               // webUI.showAlert("WARNING",  wfSubmit.getScriptMessage());
            }
            //If the node's parent is NOT in Approved workspace
            else if (styleStatus != "Approved") {
                node.getValue("a_CC_Life_Cycle_Status").setSimpleValue("Waiting for Style Approval");
            }
        }
