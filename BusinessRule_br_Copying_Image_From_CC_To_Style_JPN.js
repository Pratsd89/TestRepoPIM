...
{
  "id" : "br_Copying_Image_From_CC_To_Style_JPN",
  "type" : "BusinessCondition",
  "setupGroups" : [ "Conditions" ],
  "name" : "Copying Image Ref From CC To Style JPN Conditon",
  "description" : null,
  "scope" : "Global",
...
*/
exports.operation0 = function (stepManager,node,PrimaryProductImage) {
//The Business condition is developed as part of Japan Requirements:PPIM-7594
...
for (var i = 0; i < CCList.size(); i++) {
    ...
    if (sortOrder != null) {
        if (CCList.get(i).getValue('a_Market_Designation').getSimpleValue().indexOf("JPN") >= 0) {
...
if(sortOrderArray.length != 0){
	...
	if(!isCCSortOrderPresent){
		...
		node.getValue('a_error_message').setSimpleValue('There is no CC with Valid Market Designation Value i.e., JPN in this case.');
		return 'There is no CC with Valid Market Designation Value i.e., JPN in this case.';
