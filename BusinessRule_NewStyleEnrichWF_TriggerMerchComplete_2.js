...
{
  "id" : "NewStyleEnrichWF_TriggerMerchComplete_2",
  "type" : "BusinessAction",
  "setupGroups" : [ "Actions" ],
  "name" : "NewStyleEnrichWF_TriggerMerchComplete_2",
  "description" : "Moves Styles from 'NewStyleEntrich_WebMerch1' to 'NewStyleEnrich_WebMerch2' for both the US and Canada New Style Enrich WFs",
  "scope" : "Global",
...
*/
exports.operation0 = function (node,manager,log,compCheck) {
//PPIM 3690
//PPIM 3736
//Declare variables
...
//The following will apply if the context is US/CA and the Style is in the correct Workflow and State
if(appliesCondition1 == true){
	...
	{	
	   ...
	   var classificationType = classificationTypeHome.getLinkTypeByID("StyleToWebSubCategoryRef");	
        ...
        log.info(classificationLinkList);
            ...
            if(classificationLinkList.length == 0){
                ...
                if (CCList.size() > 0) {
                    for (var i = 0; i < CCList.size(); i++) {
            		    ...
            		    if(marketDesignation.contains('US') && lifeCycleStatus != "IN_PROGRESS"){
                            ...
                            if(isCCActive == true){
                                ...
                                log.info(ccClassificationLinkList);
            ...
            if(classificationLinkList.length == 0){
                ...
                if (CCList.size() > 0) {
                    for (var i = 0; i < CCList.size(); i++) {
                        ...
                        var marketDesignation = cc.getValue('a_Market_Designation').getSimpleValue();
                        var lifeCycleStatus = cc.getValue('a_CC_Life_Cycle_Status').getSimpleValue();
            		    if(marketDesignation.contains('CAN') && lifeCycleStatus != "IN_PROGRESS"){
                            var isCCActive = false;
                            var ccDeactivationDate = cc.getValue('a_CC_End_Date').getSimpleValue();
                            if(ccDeactivationDate == null){
                                isCCActive = true;
