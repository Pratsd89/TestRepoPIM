...
{
  "id" : "br_create_Shot_Request_Import_Actions",
  "type" : "BusinessAction",
  "setupGroups" : [ "Actions" ],
  "name" : "Create Shot Request Import Actions",
...
*/
exports.operation0 = function (step,node,ccAttr,REF_TYPE,LKT,eventType,assethubqueueUS,assethubqueueCA,assethubqueueJP,assethubqueueSA,BOM_CC_REF,photoLib) {
/*
    ...
    sharedMarkets.forEach(function (mkt) {
        ...
        step.executeInContext(nextContext, function (manager) {
            ...
            if (duplicateShot == null) {
                ...
                logArray.push("\nccNum is: " + ccNum);
                //log.info("create reference in "+nextContext+","+contextCC.getID()+contextEntity.getID())
                ...
                */
                ...
                // PPIM-13065
                brandNumber = contextCC.getValue("a_Brand_Number").getSimpleValue();
                if (brandNumber == "GO" || brandNumber == "BRFS") {
                    var BOM_CC_list = contextCC.queryReferencedBy(BOM_CC_REF).asList(100);
                    ...
                    if (BOM_CC_list.size() > 0) {
                        ...
                        step.executeInContext("EN_SA", function (otherStepManager) {
                            ...
                            SAShotCode = otherStepManager.getObjectFromOtherManager(contextEntity);
                ...
                if (brandNumber == "GAP" || brandNumber == "BR") {
                    marketDesignation = contextCC.getValue("a_Market_Designation").getSimpleValue();
                    shared_markets = contextEntity.getValue("a_Shared_Markets").getSimpleValue();
                    var currentContext = step.getCurrentContext().getID();
                    if (marketDesignation.contains("SA") && !shared_markets.contains("SA") && checkIfShotHasToBeCopied(contextCC, contextEntity, true)) {
                        step.executeInContext("EN_SA", function (otherStepManager) {
                            otherNode = otherStepManager.getObjectFromOtherManager(contextCC);
                            SAShotCode = otherStepManager.getObjectFromOtherManager(contextEntity);
                            try {
                                otherNode.createReference(SAShotCode, "CCToPhotoShotRef");
                            } catch (e) {
                    ...
                    if (marketDesignation.contains("SA") && shared_markets.contains("SA") && currentContext != "EN_SA") {

                        BOM_CC_list = contextCC.queryReferences(BOM_CC_REF).asList(100);
                        if (BOM_CC_list.size() > 0) {
                            checkAndremoveSAMarket(contextCC, contextEntity);
                            //removeSAMarket();
...
//log.info("logArray "+logArray);

function checkAndremoveSAMarket(cc, node) {
    delete_flag = false;
    step.executeInContext("EN_US", function (otherStepManager) {
        USShotCode = otherStepManager.getObjectFromOtherManager(node);
        USShotCode.queryReferencedBy(REF_TYPE).forEach(function (referenceInstance) {
    ...
    if (!delete_flag) {
        step.executeInContext("EN_JP", function (otherStepManager) {
            JPShotCode = otherStepManager.getObjectFromOtherManager(node);
            JPShotCode.queryReferencedBy(REF_TYPE).forEach(function (referenceInstance) {
    ...
    if (delete_flag) {
        step.executeInContext("EN_SA", function (otherStepManager) {
            ...
            SAShotCode.queryReferencedBy(REF_TYPE).forEach(function (referenceInstance) {
        ...
        if (new_value) {
            ...
            node.getValue("a_Shared_Markets").setSimpleValue(new_value);
        }
    }
}
