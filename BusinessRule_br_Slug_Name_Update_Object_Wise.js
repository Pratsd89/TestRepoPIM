...
{
  "id" : "br_Slug_Name_Update_Object_Wise",
  "type" : "BusinessAction",
  "setupGroups" : [ "SEO" ],
  "name" : "Slug Name Update Object Wise",
  "description" : null,
  "scope" : "Global",
  "validObjectTypes" : [ ],
...
*/
exports.operation0 = function (node,divisionSlug,queryHome,stepManager,categorySlug,subCategorySlug,Natural_Language_URL,frenchSlug,slug) {
//global variable to check the Context 
//var currentContext=stepManager.getCurrentContext().getID();


//var stiboValue = node.getValue('a_SubCategory_Slug').getSimpleValue();
...
if (objectType == "WebDivision") {
    ...
    if (Context == 'EN_US' || Context == 'EN_CA') {
        divisionSlugValue = slug.slugify(String(divisionSlugValue), {
            locale: finallocale
        });
    } else if (Context == 'FR_CA') {
        divisionSlugValue = frenchSlug.slugify(String(divisionSlugValue), {
            locale: finallocale
        });
    }
    ...
    if (Context == 'EN_US' || Context == 'EN_CA') {
         categorySlugValue = slug.slugify(String(categorySlugValue), {
            locale: finallocale
        });
    } else if (Context == 'FR_CA') {
         categorySlugValue = frenchSlug.slugify(String(categorySlugValue), {
            locale: finallocale
        });
...
//else if (objectType == "WebSubCategory" && children.length==0 && node.getParent().getValue('a_Natural_Language_URL').getSimpleValue()!=null) {
    ...
    if (Context == 'EN_US' || Context == 'EN_CA') {
        subCategorySlugValue = slug.slugify(String(subCategorySlugValue), {
            locale: finallocale
        });
    } else if (Context == 'FR_CA') {
        subCategorySlugValue = frenchSlug.slugify(String(subCategorySlugValue), {
            locale: finallocale
        });
