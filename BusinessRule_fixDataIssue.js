...
for (var ccIndex = 0; ccIndex < ccList.size(); ccIndex++) 
{
	var cc = ccList.get(ccIndex);
	...
	step.executeInContext("EN_US", function (usstep) {
		var usProduct = usstep.getProductHome().getProductByID(cc.getID());
	...
	step.executeInContext("EN_CA", function (castep) {
		var caProduct = castep.getProductHome().getProductByID(cc.getID());
	...
	if (caACLMarketDesig != null || usACLMarketDesig != null) {
		if (caACLMarketDesig == null) {
			cc.getValue("a_Market_Designation").setSimpleValue(usACLMarketDesig);
				if(cc.getParent().getValue("a_Style_Market_Designation").getSimpleValue() == null){
					cc.getParent().getValue("a_Style_Market_Designation").setSimpleValue(usACLMarketDesig);
				}
				else{
					var marDes=cc.getParent().getValue("a_Style_Market_Designation").getSimpleValue();
					if(marDes.contains("CAN") && (!(marDes.contains("US")))){
						cc.getParent().getValue("a_Style_Market_Designation").addLOVValueByID("1");
					}
								
				}
		} else if (usACLMarketDesig == null) {
			cc.getValue("a_Market_Designation").setSimpleValue(caACLMarketDesig);
				if(cc.getParent().getValue("a_Style_Market_Designation").getSimpleValue() == null){
					cc.getParent().getValue("a_Style_Market_Designation").setSimpleValue(caACLMarketDesig);
				}
				else{
					var marDes=cc.getParent().getValue("a_Style_Market_Designation").getSimpleValue();
					if(marDes.contains("US") && (!(marDes.contains("CAN")))){
						cc.getParent().getValue("a_Style_Market_Designation").addLOVValueByID("4");
					}							
				}
		} else {
			var marketdesig = caACLMarketDesig + '<multisep/>' + usACLMarketDesig;
			cc.getValue("a_Market_Designation").setSimpleValue(marketdesig);
			cc.getParent().getValue("a_Style_Market_Designation").setSimpleValue(marketdesig);
		}
	}
	...
	var lastModifiedUser = cc.getRevision().getUserID();
		...
		if (marketDesignation.contains("CAN")) {
			step.executeInContext('EN_CA', function (caContextManager) {
				...
				if (Override_CC_Name == null) {
					...
					//log.info("a_Override_CC_Name  " + cc.getValue("a_Override_CC_Name").getSimpleValue());
				}
			})
		}
	}
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//Style Imported
	var style = cc.getParent().getID();
	var CC_Country_Origin = cc.getValue("a_CC_Country_of_Origin").getSimpleValue();
	...
	var objType = cc.getObjectType().getID();
	if (objType == "CustomerChoice") {
		var marketDesignation = cc.getValue("a_Market_Designation").getSimpleValue();
		if (marketDesignation != null) {
			if (marketDesignation.contains("US")) {
				updateMarketDesigShotReq(cc, "EN_US");
			}
			if (marketDesignation.contains("CAN")) {
				updateMarketDesigShotReq(cc, "EN_CA");
			}
		}
	}
	/////////////////////////////////////////////////End Block/////////////////////////////////////////////
	...
	var objType = cc.getObjectType().getID();
	if (objType == "CustomerChoice") {
		var marketDesignation = cc.getValue("a_Market_Designation").getSimpleValue(); //it may contain US, CAN or US/CAN
		if (marketDesignation != null) {
			if ((currentContext == "EN_US" && marketDesignation.contains("US")) || ((currentContext == "EN_CA" || currentContext == "FR_CA") && marketDesignation.contains("CAN"))) {
				var status = cc.getValue("a_Source_CC_Life_Cycle_Status").getSimpleValue();
				if (status == "Draft") {
					order = 1;
				  //  triggerCCEnrichmentWorkflow(cc, order);
					...
					shotRef.addAll(cc.getReferences(shotRequestRef));
				...
				//} else if ((status == "Approved" && (cc.isInWorkflow("wf_CCEnrichment") || cc.isInWorkflow("wf_CCEnrichmentCanada"))) || (status == "Approved" && cc.getApprovalStatus() == "Not in Approved workspace")) {
				//To Cover PPIM-1802 & 2940 cases. Removing approval status check
				} else if (status == "Approved" && cc.getValue("a_CC_Life_Cycle_Status").getSimpleValue() != "Approved") {
					order = 6;
				  //  triggerCCEnrichmentWorkflow(cc, order);
				} else if (status == "Approved" && cc.getValue("a_CC_Life_Cycle_Status").getSimpleValue() == "Approved" && !cc.isInWorkflow("wf_CCEnrichment") && !cc.isInWorkflow("wf_CCEnrichmentCanada") && cc.getApprovalStatus() != "Not in Approved workspace") {
					cc.approve();
				}
				...
				if (order != 0) {
					if (currentContext == "EN_US") {
						triggerCCEnrichmentWorkflow(cc, order, "wf_CCEnrichment");
					} else {
						triggerCCEnrichmentWorkflow(cc, order, "wf_CCEnrichmentCanada");
					}
				}
				
				
			} else {
				cc.getValue("a_error_message").setSimpleValue(step.getCurrentContext().getID() + " : a_Market_Designation is not valid for executing context");
			}
		} else {
			cc.getValue("a_error_message").setSimpleValue(step.getCurrentContext().getID() + " : a_Market_Designation is missing");
		}
	}
}
