...
{
  "id" : "SetProductPastDraftDeadlineJP",
  "type" : "BusinessAction",
  "setupGroups" : [ "Actions" ],
  "name" : "SetProductPastDraftDeadlineJP",
  "description" : null,
  "scope" : "Global",
  "validObjectTypes" : [ ],
...
deadlineTime = node.getTaskByID("wf_NewStyleEnrichmentJapan", "NewStyleEnrichState1").getDeadline().getTime();
...
if(parseInt(deadlineTime) < parseInt(currTime)){
	...
	deadlineTime = node.getTaskByID("wf_CCEnrichmentJapan", "NewCCEnrichState1").getDeadline().getTime();
	...
	if(parseInt(deadlineTime) < parseInt(currTime)){
		...
		deadlineTime = node.getTaskByID("wf_NewSKUEnrichmentJapan", "NewSKUEnrich1").getDeadline().getTime();
