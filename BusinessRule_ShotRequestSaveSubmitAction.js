...
{
  "id" : "ShotRequestSaveSubmitAction",
  "type" : "BusinessAction",
  "setupGroups" : [ "Actions" ],
  "name" : "ShotRequestSaveSubmitAction",
  "description" : null,
  "scope" : "Global",
...
if (brandNumber == "GAP" || brandNumber == "BR") {
    ...
    var currentContext = step.getCurrentContext().getID();
    if (marketDesignation.contains("SA") && !shared_markets.contains("SA") && checkIfShotHasToBeCopied(cc, node, true)) {
        step.executeInContext("EN_SA", function (otherStepManager) {
    ...
    if (marketDesignation.contains("SA") && shared_markets.contains("SA") && currentContext != "EN_SA") {
        ...
        if (BOM_CC_list.size() > 0) {
            checkAndremoveSAMarket(cc, node);
            //removeSAMarket();
