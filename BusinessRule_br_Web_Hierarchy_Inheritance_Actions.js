...
{
  "id" : "br_Web_Hierarchy_Inheritance_Actions",
  "type" : "BusinessAction",
  "setupGroups" : [ "Actions" ],
  "name" : "Web Hierarchy Inheritance Actions",
  "description" : null,
  "scope" : "Global",
  "validObjectTypes" : [ "CMS_Content_Group", "CMS_Slot", "WebCategory", "WebDivision", "WebSubCategory" ],
...
*/
exports.operation0 = function (node,step,webUI,LKT,setNewSortOrders,publishChildCats,slug) {
                ...
                if (objectType != "WebDivision") {
                    if (activeCat == false) {
                        ...
                        if (usParentStart == null || usParentEnd < today) {
                            ...
                            usNode.getValue(inheritAtt).setValue(null);
                            webUI.showAlert("ERROR", "<b>" + nlumsg + "Inherit Option Removed:</b>", "<b>The parent " + usNode.getParent().getObjectType().getName() + " is not Active in the US market. Please adjust start/end date of parent before activating this category.</b>");
                            return;
                        }
                        if (sortOrder == "999999") {
                ...
                usManager.executeInContext(engContext, function (otherManager) {
                    ...
                    if (cntxtValue != attributeValue) {
                        if (currentContext == engContext) {
                            ...
                            webUI.showAlert("WARNING", "<b>" + nlumsg + "Modification in context " + currentContext + ", other than Redirect URL, will not apply, as this category is inheriting values from the US market. Please switch to EN_US to modify this category.</b>");
                        }
                    ...
                    if (objectType != "WebDivision") {
                        ...
                        if (cntxtParentStart == null || cntxtParentEnd < today) {
                            ...
                            webUI.showAlert("ERROR", "Error", "<b>" + nlumsg + "Inherit Option was removed, as the parent " + cntxtNode.getParent().getObjectType().getName() + " is not Active in the " + cntxtMarket + " market. Please adjust start/end date of parent before activating this category.</b>"); return;
                        }
...
function setSortOrderInCurrentMarket(node, manager) {
    ...
    //PPIM-11055 : Date formatted to yyyy-MM-dd
    var formatter = java.time.format.DateTimeFormatter.ofPattern("yyyy-MM-dd");
    today = today.format(formatter);
    ...
    if (objectType != "WebDivision") {
        ...
        if (catStart == null || catEnd < today) {
            ...
            if (catSortOrder == "999999") {
                ...
                if (currentMarket != "US" && currentMarket != "SA") {
                    //                	log.info("heree")
...
if (objectType == "WebDivision") {
    ...
    var currentContext = step.getCurrentContext().getID();
    var imagePlacement = node.getValue("a_Image_Placement").getSimpleValue();
    if (imagePlacement == null || imagePlacement == "") {
        node.getValue("a_Image_Placement").setSimpleValue("Main P1");
    }
    if (currentContext.equals("EN_US")) {
        var canInherit = node.getValue("a_CAN_Inherit_Option").getSimpleValue() != null;
        if (canInherit) {
            var ENUS_redirectUrl = null;
            var ENCA_redirectUrl = null;
            var ENUS_CatDisplayType = null;
            var ENCA_CatDisplayType = null;
            var ENUS_imgPlc = null;

            step.executeInContext("EN_US", function (manager) {
                var ENUS_node = manager.getObjectFromOtherManager(node);
                ENUS_imgPlc = ENUS_node.getValue("a_Image_Placement").getSimpleValue();
                ENUS_redirectUrl = ENUS_node.getValue("a_Redirect_URL").getSimpleValue();
                ENUS_CatDisplayType = ENUS_node.getValue("a_Category_Display_Type").getSimpleValue();
            });

            step.executeInContext("EN_CA", function (manager) {
                var ENCA_node = manager.getObjectFromOtherManager(node);
                ENCA_node.getValue("a_Image_Placement").setSimpleValue(ENUS_imgPlc);
                ENCA_redirectUrl = ENCA_node.getValue("a_Redirect_URL").getSimpleValue();
                ENCA_CatDisplayType = ENCA_node.getValue("a_Category_Display_Type").getSimpleValue();

                if (((ENCA_redirectUrl == null || ENCA_redirectUrl == "") && (ENUS_redirectUrl != null && ENUS_redirectUrl != "") && ENUS_CatDisplayType == "Standard: Content") || ((ENUS_CatDisplayType != "Standard: Content" && ENCA_CatDisplayType == "Standard: Content") && (ENUS_redirectUrl == null || ENUS_redirectUrl == ""))) {
                    ENCA_node.getValue("a_Redirect_URL").setSimpleValue(ENUS_redirectUrl);
                }
            });
        }
    }
...
if (currentMarket != "JPN") {
    if (inheritCAN == "Attributes") {
        ...
        step.executeInContext("EN_US", function (contextManager) {
            var usNode = contextManager.getObjectFromOtherManager(node);
            var usSortOrder = usNode.getValue("a_WebCategory_Sort_Order").getSimpleValue();
            contextManager.executeInContext("EN_CA", function (caContextManager) {
                var caNode = caContextManager.getObjectFromOtherManager(node);
                caNode.getValue("a_WebCategory_Sort_Order").setSimpleValue(usSortOrder);
            });
        });
...
if (currentMarket != "CAN") {
    if (inheritJPN == "Attributes") {
        ...
        step.executeInContext("EN_US", function (contextManager) {
            var usNode = contextManager.getObjectFromOtherManager(node);
            var usSortOrder = usNode.getValue("a_WebCategory_Sort_Order").getSimpleValue();
            contextManager.executeInContext("EN_JP", function (jpContextManager) {
                var jpNode = jpContextManager.getObjectFromOtherManager(node);
                jpNode.getValue("a_WebCategory_Sort_Order").setSimpleValue(usSortOrder);
            });
        });
...
if (setSortToTopMkts != null) {
    ...
    webUI.showAlert("WARNING", "<b>" + nlumsg + "This category was sorted to top of list for parent " + parentObjType + " in Markets " + setSortToTopMkts + ". Please navigate to the parent " + parentObjType + " and adjust sort orders, if needed.</b>");
}
else if (missingSortMkts != null) {

    webUI.showAlert("WARNING", "<b>" + nlumsg + "This " + objectType + " does not have a Sort Order value in " + missingSortMkts + ". Please navigate to the parent " + parentObjType + " and adjust sort order accordingly, for each market.</b>");
}
}
/*===== business rule plugin definition =====
{
