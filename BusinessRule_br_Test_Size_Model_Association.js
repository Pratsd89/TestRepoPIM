...
{
  "id" : "br_Test_Size_Model_Association",
  "type" : "BusinessAction",
  "setupGroups" : [ "Actions" ],
  "name" : "br_Test_Size_Model_Association",
  "description" : null,
  "scope" : "Global",
    ...
    if (obj == "Style") {
        ...
        if (id_classification_STYLE != null) {
            var classification_STYLE = manager.getClassificationHome().getClassificationByID(id_classification_STYLE);
                    log.info("classification_STYLE"+classification_STYLE);
            ...
            if (classification_STYLE != null) {
  			...
  			var styleLifeCycleststatus= node.getValue("a_Style_Life_Cycle_Status").getSimpleValue();
                log.info("styleLifeCycleststatus"+styleLifeCycleststatus);
                log.info("id_classification_STYLE"+id_classification_STYLE);
                   log.info("classification_STYLE"+classification_STYLE);
                if ((sClassID != null) && (styleLifeCycleststatus != "Approved") && (id_classification_STYLE != sClassID)){
                	log.info("reacheddddddddddddddd");
            ...
            //checking if the new size model is not available and checking for that style any reference available  
			else if (classification_STYLE == null) {
                log.info("reached else if");
 			...
 			if ( styleLifeCycleststatus != "Approved" ){
                	log.info("reachedddddddddddddddSMNULL");
                	updateSizeModel = true;               
                }
            ...
            log.info("classification_STYLE"+classification_STYLE);
            log.info("reachedddd2");
            var refs = new java.util.ArrayList();
                log.info("reachedddd22");
                ...
                if (CC_ID != null) {
                    ...
                    for (var j = 0; j < refs1.size(); j++) {
                        ...
                        var id_classification_SKU = SKU_ID.getValue("a_Size_Code").getSimpleValue();
                         //log.info("id_classification_SKU"+id_classification_SKU);
                       ...
                       log.info("reachedddd3");
                       log.info("id_classification_SKUss"+id_classification_SKU);
                        if (id_classification_SKU != null) {
                           log.info("reachedddd4");
                            var sub_class = null;
                                        ...
                                        while (chItersk.hasNext()) {
                                            ...
                                            if(linksk == "SKUToSizeCode"){
                                            	  chprodsk_ID = chprodsk.getClassification().getName();
                                            	    log.info("chprodsk_ID"+chprodsk_ID);
                                            	  parentIDsk = chprodsk.getClassification().getParent().getID();
                                            	  log.info("parentIDsk"+parentIDsk);
                                            }
                                        }
                                    }
							log.info("id_classification_SKU"+id_classification_SKU);
							log.info("ref_SKU"+ref_SKU);
							log.info("parentIDsk,newSizeModel"+parentIDsk+newSizeModel);
                                    if ((chItersk.hasNext() == false) || (chprodsk_ID != id_classification_SKU) || (parentIDsk != newSizeModel)) {
                                        try {  
                                            ...
                                            if (e.javaException instanceof com.stibo.core.domain.UniqueConstraintException) {
                                                log.warning("Link between product and classification with already existsssss.");
                                            } else {
                                                throw e;
                                            }
                                         } }
                            ...
                            if (skuSizeCodeLinkCreated == false) {
                                if (classifications_SKUs == null) {
                                    ...
                                    log.info("Size Code Creation done");
