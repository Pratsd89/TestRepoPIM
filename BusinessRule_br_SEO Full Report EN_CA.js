...
{
  "id" : "br_SEO Full Report EN_CA",
  "type" : "BusinessAction",
  "setupGroups" : [ "SEO" ],
  "name" : "br_SEO Full Report EN_CA",
  "description" : null,
  "scope" : "Global",
  "validObjectTypes" : [ ],
...
fw.write("ID,CID,Brand,CategoryStartDate,CategoryEndDate,WebCategoryHide,ObjecType,EN_CA_NLU,EN_CA_Legay_LIVEURL,EN_CA_NLU_LIVEURL\n");
...
function generatePreviewNLULegacyURLCategory(brandInput, nluInput, cidInput, legacy, context) {
	...
	if (context == "EN_CA") {
		...
		var temp3 = "&locale=fr_CA&inventoryAware=true&previewDate=2024-09-19%2016%3A14%3A00%20PDT";
	}
	if (legacy == 'LEGACY') {
...
function getDivisionDetails(divisiondObj, divContext,contextID) {		//Division ID
	...
	var en_CA_nluLive = generatePreviewNLULegacyURLDivisionLIVE(brandNo, en_CA_divslugNLU, divisionCID, "PREVIEW", "EN_CA");
	...
	var en_CA_legacyLive = generatePreviewNLULegacyURLDivisionLIVE(brandNo, en_CA_divslugNLU, divisionCID, "LEGACY", "EN_CA");
	...
	fw.write(divisionId + "," +divisionCID+ "," +brandNo+ "," +ctyStartDate+ "," +ctyEndDate+ "," +categoryHide+ ","+objectTypeName+ ","+en_CA_divslugNLU + "," +en_CA_legacyLive+"," +en_CA_nluLive+ "\n");
}
//################################################################
...
function getCategoryDetails(ctyObject, ctyContext,contextID) {
	...
	var en_CA_nluLive = generatePreviewNLULegacyURLCategoryLIVE(catBrandNo, en_CA_ctySlugNLU, ctyCID, "PREVIEW", "EN_CA");
    ...
    var en_CA_legacyLive = generatePreviewNLULegacyURLCategoryLIVE(catBrandNo, en_CA_ctySlugNLU, ctyCID, "LEGACY", "EN_CA");
    ...
    Name = cleanString(Name);
     ...
     var frenchParentName = frenchObject.getParent().getName();
     frenchParentName = stringOperations(frenchParentName);
     fw.write(ctyId+ "," +ctyCID + "," +catBrandNo+","+ctyStartDate+","+ctyEndDate+","+categoryHide+ "," +objectTypeCty+ ","+en_CA_ctySlugNLU + "," +en_CA_legacyLive+"," +en_CA_nluLive + "\n");
}//################################################################
//********** write Category details  ***********
//################################################################
function getSubCategoryDetails(subCtyObject, subCtyContext,contextID) {
    ...
    var en_CA_nluLive = generatePreviewNLULegacyURLSubCategoryLIVE(subCtyBrandNo, en_CA_subCtySlugNLU, subCtyCID, subCtyCtyCID, "PREVIEW", "EN_CA");
	...
	var en_CA_legacyLive = generatePreviewNLULegacyURLSubCategoryLIVE(subCtyBrandNo, en_CA_subCtySlugNLU, subCtyCID, subCtyCtyCID, "LEGACY", "EN_CA");
	...
	fw.write(subCtyid + "," + subCtyCID + "," +subCtyBrandNo+ "," +ctyStartDate+","+ctyEndDate+","+categoryHide+ "," +objectTypeSubCty+ "," +en_CA_subCtySlugNLU+ "," + en_CA_legacyLive+"," +en_CA_nluLive+ "\n");
}
...
var emailIDTO = mailMethod.addTo("sri_indu_dekkapati@gap.com;jagadish_beejapu@gap.com;aditya_raj_singh@gap.com");
var emailSubject = mailMethod.subject("Full Report - "+ contextID);
var emailBody = mailMethod.plainMessage("Please find NLU URL's for "+ contextID);
// set attachment 
var attachment = mailMethod.attachment();
var fromAsssest = attachment.fromAsset(asset);
var setAttachMentName = attachment.name("SEO-PIM-Report_For_Legacy_NLU_EN_CA.csv");
attachment.attach();
//send email 
...
function mainMethod(context) {
	...
	var brand_ALL = stepManager.getClassificationHome().getClassificationByID(WBU_ID[i])
