...
{
  "id" : "br_bulk_Shot_Request_Import_Actions_temp",
  "type" : "BusinessAction",
  "setupGroups" : [ "Actions" ],
  "name" : "Bulk Shot Request Import Actions temp",
...
*/
exports.operation0 = function (step,node,ccAttr,REF_TYPE,LKT,eventType,assethubqueueUS,assethubqueueCA,assethubqueueJP,assethubqueueSA,helper,photoLib) {
/*
...
//var x = node.getValue("a_Shared_Markets").getValues().toArray();
//log.info(x.contains("US"))
...
ccArray = new java.util.HashSet();
if (sharedMarketsString.contains('US') == true) {
    var ccList = getCCsFromShot(node, 'EN_US')
    //log.info("IN US " + ccList);
    ccArray.add(ccList)
}
else if (sharedMarketsString.contains('CAN')) {
    var ccList = getCCsFromShot(node, 'EN_CA')
    //log.info("IN CA " + ccList);
    cc = ccList
    ccArray.add(ccList)
}
else if (sharedMarketsString.contains('JPN')) {
    var ccList = getCCsFromShot(node, 'EN_JP')
    //log.info("IN JP " + ccList);
    cc = ccList
    ccArray.add(ccList)
}
if (sharedMarketsString.contains('SA')) {
    log.info("IN SA " + ccList);
    var ccList = getCCsFromShot(node, 'EN_SA')
    cc = ccList
    ccArray.add(ccList)
}
...
log.info("cc array "+ccArray.size())
if (sharedMarketsString.contains('SA')) {
    sharedMarkets.forEach(function (mkt) {
        found = false;
        var Context = LKT.getLookupTableValue("LKT_MarketDesignationToMarket", mkt.getValue());
        log.info(mkt.getValue())
        step.executeInContext(Context, function (otherContextManager) {
            var cc1 = getCCsFromShot(node, Context)
            var contextShotRequest = otherContextManager.getEntityHome().getEntityByID(node.getID());
            var contextShotRequestStatus = contextShotRequest.getValue("a_Shot_Request_Lifecycle_Status").getSimpleValue();
            var contextShotRequestPlacement = contextShotRequest.getValue("a_Site_Placement").getSimpleValue();
            var contextShotRequestMarkets = contextShotRequest.getValue("a_Shared_Markets").getSimpleValue();
            log.info(Context + ","+cc1 + " cc1")
            if (cc1) {
                var cc_market_designation = cc1.getValue("a_Market_Designation").getValues().toArray()
                log.info(cc_market_designation+" cc_market_designation")
                cc_market_designation.forEach(function (y) {
                	log.info(y.getValue())
                    if (y.getValue() == mkt.getValue()) {
                        found = true;
                        return false;
                    }
                });
            ...
            if (!found) {
                if (cc1) {
                    //delete the link
                    deleteCCShotRequestLink(node, Context)
                    log.info("link deleted")

                }
                contextShotRequest.getValue("a_Shot_CC_Number").setSimpleValue(null);
                contextShotRequest.getValue("a_Shot_Market_Number").setSimpleValue(null);
                //check if cc's in other markets have this market.
                matchedCC = checkForMarketInCCs(ccArray, mkt);
                if (matchedCC != "") {
                    found = true;
                    cc1 = otherContextManager.getProductHome().getProductByID(matchedCC.getID());
                    var CCPhotoStatus = cc1.getValue("a_CC_Photo_Status").getSimpleValue();
                    cc1.createReference(contextShotRequest, REF_TYPE);
                    if (contextShotRequestPlacement == "Main P1" && contextShotRequestStatus == "Submitted") {
                        getWorkflowVariables(matchedCC, marketValue, contextShotRequestStatus);
                    }
                    else if (contextShotRequestStatus == "Submitted") {
                        if (CCPhotoStatus == "Complete") {
                            matchedCC.getValue('a_CC_Photo_Status').setSimpleValue("Complete: Request Submitted");
                        }
                    }
                    else if (contextShotRequestStatus == "Ready for Review") {
                        if (CCPhotoStatus == "Complete" || CCPhotoStatus == "Complete: Request Submitted") {
                            matchedCC.getValue('a_CC_Photo_Status').setSimpleValue("Complete: Ready for Review");
                        }
                    }
                }
                else{
            ...
            if(found) {
                ...
                contextShotRequest.getValue("a_Shot_Market_Number").setSimpleValue(mkt.getID() + " : " + mkt.getValue());
                var ccNum = cc1.getValue("a_CC_Number").getSimpleValue();
                contextShotRequest.getValue("a_Shot_CC_Number").setValue(ccNum);
 			...
 			*/
                ...
                if (sCCArray != null && stylingPieceInstrArray != null) {
                    ...
                    for (x = 0; x < sCCArray.length; x++) {
                        ...
                        var contextsCC = otherContextManager.getProductHome().getProductByID(sCC.getID());
                        ...
                        createShotRequestToStylingPieceCCRef(contextShotRequest, contextsCC, stylingPieceInstrArray[x]);
                   }
                }
            ...
            if (found ){
            //&& changedAttrs.contains("a_Site_Placement") == false) {
                if (contextShotRequestStatus == "Ready for Review" || contextShotRequestStatus == "Complete") {
                    log.info("\n -- Running library function updateContentReferencesForShotAndCC for " + contextShotRequest.getID() + ", " + otherContextManager);
                    photoLib.updateContentReferencesForShotAndCC(contextShotRequest, otherContextManager, Context, true);
                }
            }

            if (found ){
            //	|| changedAttrs.contains("a_Site_Placement") == true) {
            
                if (contextShotRequestPlacement == "Main P1") {
                    getWorkflowVariables(cc1, mkt.getValue(), contextShotRequestStatus);
                }
            }
        });
    })
...
if (ccMarkets != null) {
    ...
    ccMarkets.forEach(function (mkt) {
        ...
        step.executeInContext(nextContext, function (manager) {
            ...
            var contextEntityMarkets = contextEntity.getValue("a_Shared_Markets").getSimpleValue();
            ...
            if (contextCC != null) {
                ...
                // if CC contains the current market
                else {
                    // set shot market number on the context specific shot request
                ...
                if (marketRemoved == false || changedAttrs.contains("a_Site_Placement") == true) {
...
if (sharedMarketsString == null) {
    //log.info("\n -- DELETING " + node);
    node.delete();
    throw "Unable to create Shot Request as CC " + cc.getValue("a_CC_Number").getSimpleValue() + "has no applicable Markets.";
}
...
if (node.isInState("wf_ShortRequestLifeCycle", "Draft") && primaryCCStatus != null && primaryCCStatus != "Draft" && primaryCCStatus != "Purged") {
...
function getCCsFromShot(node, Context) {
    log.info(Context + " Context")
    cc1 = ""
    step.executeInContext(Context, function (otherContextManager) {
        var contextEntity = otherContextManager.getEntityHome().getEntityByID(node.getID());
        var ccList = contextEntity.queryReferencedBy(REF_TYPE).asList(100);
        log.info("ccList " + ccList.size())
        if (ccList.size() > 0) {
            log.info("cccc")
            cc1 = ccList.get(0).getSource();
        }
    });
    return cc1;
}
function deleteCCShotRequestLink(node, Context) {
    step.executeInContext(Context, function (otherContextManager) {
        var contextEntity = otherContextManager.getEntityHome().getEntityByID(node.getID());
        var ccList = contextEntity.queryReferencedBy(REF_TYPE);
        ccList.forEach(function (link) {
            link.delete()
        })
        //log.info(contextEntity.queryReferencedBy(REF_TYPE).asList(10).size())
    });

    return true;;
}

function checkForMarketInCCs(ccArray, mkt) {
    matchedCC = "";
    ccArray.forEach(function (cc) {
        var cc_market_designation = cc.getValue("a_Market_Designation").getValues().toArray()
        cc_market_designation.forEach(function (y) {
            if (y.getValue() == mkt.getValue()) {
                matchedCC = cc;
                return false;
