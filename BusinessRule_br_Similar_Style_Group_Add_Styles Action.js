...
{
  "id" : "br_Similar_Style_Group_Add_Styles Action",
  "type" : "BusinessAction",
  "setupGroups" : [ "Actions" ],
  "name" : "Similar Style Group Add Styles Action",
  "description" : "PPIM-11288, PIM-11365",
  "scope" : "Global",
...
*/
exports.operation0 = function (node,step,MergeSimilarStyles,LKT,styles,webUI) {
function inheritAttributes(primaryStyle, supportingStyle, manager) {
    var inheritATG = manager.getAttributeGroupHome().getAttributeGroupByID("ag_SuperPDP_ProductTags_Inherit_Attrs");
...
function createSSGStyleReference(contextid, step) {
    ...
    var reference = contextNode.createReference(contextCurrentStyle, MergeSimilarStyles);
    ...
    contextPrimarySellingStyle.getValue("a_Supporting_Styles").addValue(currentStyle.getID());
...
if (context == "EN_CA" && superPDPMarket != null && superPDPMarket.contains("US")) {
    ...
    if (styles == null) {
        ...
        for (var i = 0; i < styleList.length; i++) {
            ...
            var CanadaSSG = getDuplicateStyleGroup(canadaCurrentStyle, refType);
            ...
            if (currentStyle == null) {
                ...
                if (parentClass.getID() == currentStyleClass.getID()) {
                    ...
                    var flagSSG = false;
                    var flagDSG = false;
                    var refBySSG = currentStyle.getReferencedBy().toArray();
                    for (var j = 0; j < refBySSG.length; j++) {
                        if (refBySSG[j].getReferenceType().getID() == "rt_styles") {
                            flagSSG = true;
                        } else if (refBySSG[j].getReferenceType().getID() == "rt_mergeDuplicateStyles") {
                            flagDSG = true;
                        }
                    }
                    ...
                    if (flagSSG) {
                        ...
                        logArray3.push(currentStyle.getValue("a_Style_Number").getSimpleValue());
                    } else {
                        // Creating Reference in Canada Market when Styles have been added to US Market - PIM-11365
                        if (context == "EN_US") {
                        createSSGStyleReference("EN_US", step);
                        if (CanadaSSG == null && superPDPMarket.contains("CAN")) {
                                createSSGStyleReference("EN_CA", step);
                            }
                    
                    } else {
                        createSSGStyleReference(context, step);
                    }
                }
                }else {
        ...
        if (logArray1.length > 0) {
            ...
            webUI.showAlert("ERROR", "These Style(s) are already a part of SSG: ", logArray2);
        } else if (logArray3.length > 0) {
            webUI.showAlert("ERROR", "These Style(s) cannot be a part of SSG as they are already a part of DSG: ", logArray3);
        }
    }
}
}