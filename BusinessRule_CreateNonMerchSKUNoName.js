...
{
  "id" : "CreateNonMerchSKUNoName",
  "type" : "BusinessAction",
  "setupGroups" : [ "Actions" ],
  "name" : "CreateNonMerchSKUNoName",
  "description" : null,
  "scope" : "Global",
  "validObjectTypes" : [ ],
...
*/
exports.operation0 = function (node,step,log,portal,linkType,refclass,LKT) {
/**
 * JP Changes are added as part of PPIM-7541
 */
//Create Non Merch SKU - PPIM 1406
var result = true;

//PPIM-2719 Create Non merch SKU without prompting for Name
...
if (result == true) {
    ...
    //PPIM-2719 default Size Code Name as the SKU Name and SKU Non Merch Price
    sSKU.setName(sizeCode);
    if (!isNaN(sizeCode)) {
    ...
    /*
    sSKU.getValue("a_Size_Dim1_Description").setSimpleValue(dim1Desc);
    sSKU.getValue("a_Size_Dim2_Description").setSimpleValue(dim2Desc);
    //set Dim1 & Dim2 Desciption in other Language context of current Market only
    //so for EN_US context no extra operation needed, but for CA market cross Language operation needed
    var crossContext1 = '';
    if (currentContext == "EN_CA") {
        crossContext1 = 'FR_CA';
    } else if (currentContext == "FR_CA") {
        crossContext1 = 'EN_CA';
    }

    if (currentContext == "EN_CA" || currentContext == "FR_CA") {
        //a_Size_Dim1_Description, a_Size_Dim2_Description & SizeCode.a_SizeCodeVariant are Market; Language
        //dependent, so need to set for current Market other (cross) Languages 
        step.executeInContext(crossContext1, function (crossContextManager) {
            var crsCtxSKU = crossContextManager.getProductHome().getProductByID(skuID);
            var crsCtxSizeCode = crossContextManager.getClassificationHome().getClassificationByID(sizeCodeId);
            var crsCtxSizeCdVar = crsCtxSizeCode.getValue("a_SizeCodeVariant").getSimpleValue();
            var crsCtxDim1Desc = "";
            var crsCtxDim2Desc = "";
            if (crsCtxSizeCdVar == null) {
                crsCtxSizeCdVar = "";
            }

            if (crsCtxSizeCode.getChildren() != null) {
                for (var i = 0; i < crsCtxSizeCode.getChildren().size(); i++) {
                    var dimObj = crsCtxSizeCode.getChildren().get(i);
                    //log.info("Object Type - dimObj= " + dimObj.getObjectType().getID() + " | ID= " + dimObj.getID() + " | Dimension= " + dimObj.getName());
                    if (dimObj.getObjectType().getID() == "Dim1") {
                        crsCtxDim1Desc = crsCtxSizeCdVar + " " + dimObj.getValue("a_Advanced_Dimension_Value").getSimpleValue();
                    } else if (dimObj.getObjectType().getID() == "Dim2") {
                        crsCtxDim2Desc = crsCtxSizeCdVar + " " + dimObj.getValue("a_Advanced_Dimension_Value").getSimpleValue();
                    }
                }
            }
            log.info("Cross context= " + crossContextManager.getCurrentContext().getID() +
                " | dim1Desc= " + crsCtxDim1Desc + " | dim2Desc= " + crsCtxDim2Desc);

            crsCtxSKU.getValue("a_Size_Dim1_Description").setSimpleValue(crsCtxDim1Desc);
            crsCtxSKU.getValue("a_Size_Dim2_Description").setSimpleValue(crsCtxDim2Desc);
        });
    }
    */
    //-----------------------------------------------------------------------
