...
/*===== business rule definition =====
{
  ...
  "type" : "BusinessAction",
  "setupGroups" : [ "Actions" ],
  "name" : "Shots_SA_InitiateWorkflowAction",
  "description" : null,
  "scope" : "Global",
...
*/
exports.operation0 = function (node,step,LKT,shotRef,av1Ref,av2Ref,av3Ref,av4Ref,av5Ref,av6Ref,av7Ref,av8Ref,av9Ref,ppiRef,swatchRef,videoRef,av10Ref,av11Ref,BOMRefType) {
...
if (currentContext != "FR_CA" && currentContext != "JA_JP") {
    ...
    if (objectType == "CustomerChoice") {
        ...
        if (currentMarket != "US" && currentMarket != "SA") {
            ...
            if (aclMarket == currentMarket && mktDsg != null && mktDsg.contains("US")) {
                ...
                step.executeInContext("EN_US", function (contextManager) {
                    ...
                    for (var i = 0; i < refs.size(); i++) {
                        ...
                        if (shared != null && shared.contains("US") && (!(shared.contains(currentMarket)))) {
        ...
        ///////////////////////////////
        else if (currentMarket == "SA") {
            var mktDsg = node.getValue("a_Market_Designation").getSimpleValue();
            if (mktDsg.contains("US")) {
                var USCheck = check(node, "EN_US");
                if (!USCheck) {
                    if (mktDsg.contains("JPN")) {
                        var JPNCheck = check(node, "EN_JP");
                        if (!JPNCheck) {
                            node.queryReferences(BOMRefType).forEach(function (refInstance) {
                                var ref_BOMcc = refInstance.getTarget();
                                check(ref_BOMcc, "EN_US");
                                return false;
                            });
                        }
                    }
        ...
        function check(cc, context) {
            step.executeInContext(context, function (ctxManager) {
                var ctxNode = ctxManager.getObjectFromOtherManager(cc);
            });
            var ctxNodeLcs = ctxNode.getValue("a_CC_Life_Cycle_Status").getSimpleValue();
            if (ctxNodeLcs != null && ctxNodeLcs != "Draft" && ctxNodeLcs != "Purged") {
                var otherCtxShotRefs = ctxNode.queryReferences(shotRef).asList(1000);
                if (otherCtxShotRefs.size() > 0) {
                    copyShots(otherCtxShotRefs, context);
                } else {
                    return false;
                }
            }
        ...
        function copyShots(refs, context) {
            var market = LKT.getLookupTableValue("LKT_Context_to_Market", context);
            for (var i = 0; i < refs.size(); i++) {
                var shot = refs.get(i).getTarget();
                var shared = shot.getValue("a_Shared_Markets").getSimpleValue();
                if (shared.contains(market) && !shared.contains("SA")) {
