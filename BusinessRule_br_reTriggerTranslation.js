...
{
  "id" : "br_reTriggerTranslation",
  "type" : "BusinessAction",
  "setupGroups" : [ "Actions" ],
  "name" : "Re-Trigger Translation Event",
...
*/
exports.operation0 = function (node,inheritUSCopy,step,DGLStandardCAN,DGLMediumCAN,DGLUrgentCAN,DGLStandardJPN,DGLMediumJPN,DGLUrgentJPN,translationEvent,LKT,webUI,globalHelper) {
...
//var PrimaryStyle = node.getValue("a_Primary_Selling_Style").getSimpleValue();
//if (PrimaryStyle == "No") {
  //  webUI.showAlert("Error",  "Cannot Trigger Translation for Supporting styles of a Duplicate Style Group");
//}
//else {
    if (urgencyValue == null) {
        webUI.showAlert("Error", "Please provide Translation Urgency value and try again.");
    }
    else {
        var urgencyID = node.getValue("a_Translation_Urgency").getLOVValue().getID();

        if (inheritOption != null) {
            inheritUSCopy.execute(node);
            ...
            if (currentMarket == "US") {
                ...
                webUI.showAlert("Error", "Incorrect Context", "Please switch to EN_CA and/or EN_JP to modfiy copy and re-trigger translations needed per Market");
            }
            else {
                var queue = "DGL" + urgencyID + currentMarket;
                globalHelper.setTranslationDueDate(node, step, currentContext);
