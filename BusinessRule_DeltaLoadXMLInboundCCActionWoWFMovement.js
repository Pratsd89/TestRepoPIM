...
{
  "id" : "DeltaLoadXMLInboundCCActionWoWFMovement",
  "type" : "BusinessAction",
  "setupGroups" : [ "Actions" ],
  "name" : "DeltaLoadXMLInboundCCActionWoWFMovement",
  "description" : null,
  "scope" : "Global",
...
*/
exports.operation0 = function (node,log,step,shotRequestRef,shotReqRef,helperLib) {
...
function updateMarketDesigShotReq(product, contextID) {
    step.executeInContext(contextID, function (contextManager) {
        ...
        if (marketDesignation != null) {
        	//Commented as part of - Changes for PPIM-7549 Start
           /* var desigValue = null;
            ...
            if (marketDesignation == "CAN") {
                desigValue = "CAN";
            } */  //Commented as part of - Changes for PPIM-7549 End
            var shotReq = null;
...
//  Changes for PPIM-7549 start
step.executeInContext("EN_JP", function (jpstep) {
    var jpProduct = jpstep.getProductHome().getProductByID(node.getID());
    if (jpProduct.getValue("a_ACL_Market_Designation").getSimpleValue() != null) {
        if (jpProduct.getValue("a_ACL_Market_Designation").isInherited() == false) {
            jpACLMarketDesig = jpProduct.getValue('a_ACL_Market_Designation').getSimpleValue();
        }
    }

});
//  Changes for PPIM-7549 end 

if (caACLMarketDesig != null || usACLMarketDesig != null || jpACLMarketDesig != null) {

	var marketDesignation=helperLib.getMarketDesignation(usACLMarketDesig,caACLMarketDesig,jpACLMarketDesig);
	node.getValue("a_Market_Designation").setSimpleValue(marketDesignation);
	
	var existingStyleMarketDesignation=node.getParent().getValue("a_Style_Market_Designation").getSimpleValue();
		if(marketDesignation!=null && existingStyleMarketDesignation!=null){
			var marketDesignationArr=existingStyleMarketDesignation.split("<multisep/>");
			for(i=0;i<marketDesignationArr.length;i++){
				if(marketDesignation.indexOf(marketDesignationArr[i])==-1)
					marketDesignation=marketDesignation+"<multisep/>"+marketDesignationArr[i]
		}
	}
	
	node.getParent().getValue("a_Style_Market_Designation").setSimpleValue(marketDesignation);
	}
/* Commented as part of Changes for PPIM-7549  added above block for same logic
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
////JPN Context/// Changes for PPIM-7549.
if (marketDesignation != null) {
    if (marketDesignation.contains("JPN")) {
        step.executeInContext('EN_JP', function (jpContextManager) {
            ...
            /////New Color:
            var CC_Start_Date = jpCurrentProduct.getValue("a_CC_Start_Date").getSimpleValue();
            //log.info(CC_Start_Date);
            ...
            if (CC_Start_Date != null) {
                ...
                if (CC_Start_Date > final_date || CC_Start_Date == final_date) {
                    jpCurrentProduct.getValue("a_New_Color").setSimpleValue("Yes");
                    //log.info(caCurrentProduct.getValue("a_New_Color").getSimpleValue());

                } else {
                    jpCurrentProduct.getValue("a_New_Color").setSimpleValue("No");
                    //log.info(caCurrentProduct.getValue("a_New_Color").getSimpleValue());
                }
            }
            //=============DONOTAllow catalog sync user to trigger functions // PPIM-3799===========
            var lastModifiedUser = node.getRevision().getUserID();
                        ...
                        ///CC Planned in DC Date
                        var CC_Source_Date = jpCurrentProduct.getValue("a_CC_Source_in_DC_Date").getSimpleValue();
                        var CC_Planned_Date = jpCurrentProduct.getValue("a_CC_Planned_in_DC_Date").getSimpleValue();
            if (lastModifiedUser.toUpperCase() != "STIBOCATALOGSYNCUSER"){
                        if (CC_Planned_Date == null && CC_Source_Date != null) {
                            jpCurrentProduct.getValue("a_CC_Planned_in_DC_Date").setSimpleValue(CC_Source_Date);
                        } else if (CC_Source_Date != null && CC_Planned_Date != null) {
                            if (CC_Source_Date < CC_Planned_Date) {
                                jpCurrentProduct.getValue("a_CC_Planned_in_DC_Date").setSimpleValue(CC_Source_Date);
                                //log.info("a_CC_Source_in_DC_Date " +enCurrentProduct.getValue("a_CC_Planned_in_DC_Date").getSimpleValue());
            ...
            ///Style IN DC Date
            var parent_style = jpCurrentProduct.getParent().getID();
            //log.info(parent_style);
            var jpCurrentParentProduct = jpContextManager.getProductHome().getProductByID(parent_style);
            var style_DC_Date = jpCurrentProduct.getValue("a_Style_IN_DC_Date").getSimpleValue();
            //log.info("date" +  style_DC_Date);
            if (style_DC_Date == null && CC_Source_Date != null) {
                jpCurrentParentProduct.getValue("a_Style_IN_DC_Date").setSimpleValue(CC_Source_Date);
            } else if (CC_Source_Date != null && style_DC_Date != null) {
                if (CC_Source_Date < style_DC_Date) {
                    jpCurrentParentProduct.getValue("a_Style_IN_DC_Date").setSimpleValue(CC_Source_Date);
                    //log.info("Style overlap" +caCurrentParentProduct.getValue("a_Style_IN_DC_Date").getSimpleValue());
            ...
            /////a_Override_CC_Name
            var Override_CC_Name = jpCurrentProduct.getValue("a_Override_CC_Name").getSimpleValue();
            if (Override_CC_Name == null) {
                jpCurrentProduct.getValue("a_Override_CC_Name").setSimpleValue("No");
                //log.info("a_Override_CC_Name  " + node.getValue("a_Override_CC_Name").getSimpleValue());
            }
        })
    }
...
if (CC_Country_Origin != null) {
    if (CC_Country_Origin == "US") {
        ...
        // Changes for PPIM-7549.
        if (marketDesignation != null) {
            if (marketDesignation.contains("JPN")) {
                step.executeInContext('EN_JP', function (jpContextManager) {
                    var jpCurrentProduct = jpContextManager.getProductHome().getProductByID(style);
                    jpCurrentProduct.getValue("a_Style_Imported").setSimpleValue("No");
                })
            }
        }
        // Changes for PPIM-7549.
        ...
        // Changes for PPIM-7549.
         if (marketDesignation != null) {
            if (marketDesignation.contains("JPN")) {
                step.executeInContext('EN_JP', function (jpContextManager) {
                    var jpCurrentProduct = jpContextManager.getProductHome().getProductByID(style);
                    jpCurrentProduct.getValue("a_Style_Imported").setSimpleValue("Yes");
                })
            }
        }
        // Changes for PPIM-7549.
...
if (objType == "CustomerChoice") {
    ...
    if (marketDesignation != null) {
        ...
        if (marketDesignation.contains("JPN")) {
            updateMarketDesigShotReq(node, "EN_JP");  // Changes for PPIM-7549.
        }
    }
}
/////////////////////////////////////////////////End Block/////////////////////////////////////////////
...
var objType = node.getObjectType().getID();
if (objType == "CustomerChoice") {
    var marketDesignation = node.getValue("a_Market_Designation").getSimpleValue(); //it may contain US, CAN or US/CAN, JPN or US/JPN
    if(marketDesignation != null){
        if(currentContext == "EN_US"){
            if (!(node.isInWorkflow("wf_CCEnrichment"))) {
                if (node.getValue("a_Source_CC_Life_Cycle_Status").getSimpleValue() != "Purged" || node.getValue("a_Source_CC_Life_Cycle_Status").getSimpleValue() != null ){
                    node.startWorkflowByID("wf_CCEnrichment", "Delta Load based initiation");
                }
            }   
        }
            ...
            if (!(node.isInWorkflow("wf_CCEnrichmentCanada"))) {
                if (node.getValue("a_Source_CC_Life_Cycle_Status").getSimpleValue() != "Purged" || node.getValue("a_Source_CC_Life_Cycle_Status").getSimpleValue() != null ){
                    node.startWorkflowByID("wf_CCEnrichmentCanada", "Delta Load based initiation");
                }
            }   
        }
        else if(currentContext == "EN_JP"){
            if (!(node.isInWorkflow("wf_CCEnrichmentJapan"))) {
                if (node.getValue("a_Source_CC_Life_Cycle_Status").getSimpleValue() != "Purged" || node.getValue("a_Source_CC_Life_Cycle_Status").getSimpleValue() != null ){
                    node.startWorkflowByID("wf_CCEnrichmentJapan", "Delta Load based initiation");
                }
            }   
        }
    }
    else {
        node.getValue("a_error_message").setSimpleValue(currentContext + " : a_Market_Designation is missing");
    }
        
    /*if (marketDesignation != null) {
    	// Changes for PPIM-7549. added JP context
        if ((currentContext == "EN_US" && marketDesignation.contains("US")) || ((currentContext == "EN_CA" || currentContext == "FR_CA") && marketDesignation.contains("CAN"))|| ((currentContext == "EN_JP" || currentContext == "JA_JP") && marketDesignation.contains("JPN"))) {
            ...
            if (order != 0) {
                ...
                } else  if (currentContext == "EN_JP" || currentContext == "JA_JP")  // Changes for PPIM-7549.
                {
                    triggerCCEnrichmentWorkflow(node, order, "wf_CCEnrichmentJapan");
                }
            }
