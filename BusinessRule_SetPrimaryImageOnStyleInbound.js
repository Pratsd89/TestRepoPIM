...
{
  "id" : "SetPrimaryImageOnStyleInbound",
  "type" : "BusinessAction",
  "setupGroups" : [ "Actions" ],
  "name" : "SetPrimaryImageOnStyleInbound",
  "description" : null,
  "scope" : "Global",
...
var sOReferencingCCs = new java.util.ArrayList();
sOReferencingCCs.addAll(node.getReferencedByProducts());
	if (!sOReferencingCCs.isEmpty()) {	
			for (var i = 0; i < sOReferencingCCs.size(); i++) {		
				var refCC = sOReferencingCCs.get(i);
				var oStyleCC = refCC.getSource();
				//log.info(oStyleCC);
			}
	}
var obj = oStyleCC.getObjectType().getID();
//log.info(oStyleCC.getParent());
if(obj == "CustomerChoice"){
var nodeP = oStyleCC.getParent();
