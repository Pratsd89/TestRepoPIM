...
while (selected.hasNext()) {
    ...
    if(CANInheritOption != null && currentContext == "EN_US") {
    	stepManager.executeInContext("EN_CA", function(currentContextManager) {
        var currentContextNode = currentContextManager.getClassificationHome().getClassificationByID(node.getID());
        Doit(currentContextNode);
        var children = currentContextNode.getChildren().toArray();
        for (var j in children) {
        	    stepManager.executeInContext("EN_CA", function(currentContextManager) {
        	    	var currentContextNode = currentContextManager.getClassificationHome().getClassificationByID(children[j].getID());
                var BusinessRuleHome = stepManager.getHome(com.stibo.core.domain.businessrule.BusinessRuleHome);
   			 BusinessRuleHome.getBusinessActionByID("br_SEOPageTitleGeneration").execute(currentContextNode);
                BusinessRuleHome.getBusinessActionByID("br_SEO_PopulatePageDescription").execute(currentContextNode);
        	    });
...
function Doit(node) {
    ...
    if (objectType == "WebCategory") {
        ...
        var isURLExists = searchExistingNaturalURL(nluCategory, Natural_Language_URL, node.getParent().getParent(),node.getID());
    ...
    if (objectType == "WebDivision") {
        var Attributes = ['a_Division_Slug', 'a_Old_Division_Slug', 'a_Natural_Language_URL','a_Category_Description'];
...
function WebSubCategory(node) { //(objectType == "WebSubCategory")
	log.info("current slug path of "+node.getID() +" is "+node.getValue("a_Natural_Language_URL").getSimpleValue());
    var stiboValue = node.getValue('a_Category_Description').getSimpleValue();
   node.getValue("a_SEO_Page_Title").setSimpleValue(null);
   node.getValue("a_SEO_Meta_Description").setSimpleValue(null);
   ...
   var slugpath = node.getValue("a_Natural_Language_URL").getSimpleValue();
    ...
    log.info("new slug path of "+node.getID()+" is "+ nluSubCategory);
    var isURLExists = searchExistingNaturalURL(nluSubCategory, Natural_Language_URL, node.getParent().getParent().getParent(),node.getID());
    ...
    if (isURLExists != 0) {
        ...
        web.showAlert("WARNING", " <b>Moved Web SubCategory " + node.getID() + " with CID " +  SubCtyCID + " with slug path "+nluSubCategory+" is having Sub Category Slug name same as one off the active Web sub category, Please review slug name by updating category description for Web Sub Category "+ node.getID()+".</b>");
        node.getValue('a_WebCategory_Note').setSimpleValue(stiboValue);
        node.getValue('a_Category_Description').setSimpleValue(null);
        node.getValue('a_Division_Slug').setSimpleValue(null);
        node.getValue("a_Natural_Language_URL").setSimpleValue(null);
...
function searchExistingNaturalURL(nluValue, Attribute, Parent,currentNodeId) {
    ...
    size = result.asList(500).size();
    if(size ==1){
        if(result.asList(500).get(0).getID() == currentNodeId){
            return 0;
        }
    }
    return size;
...
function CopyWebCategoryAttributes(node) {
    var Attributes = ["a_Category_Slug", "a_Old_Category_Slug", "a_Natural_Language_URL", "a_Web_Catergory_Start_Time",'a_Category_Description'];
    for (var m in Attributes) {
        CopyAttributeValues(node, Attributes[m]);
    }
    var children = node.getChildren().toArray();
