...
{
  "id" : "br_Export_Web_Hierarchy_Archive_Details2",
  "type" : "BusinessAction",
  "setupGroups" : [ "SEO" ],
  "name" : "br_Export_Web_Hierarchy_Archive_Details2",
  "description" : null,
  "scope" : "Global",
  "validObjectTypes" : [ ],
...
exports.operation0 = function (stepManager,node,mail) {
	...
	fw.write("ID,Division Name,CA Division Name,French Division Name,Division CID,Category Name,CA Category Name,French Category Name,Category CID,SubCategory Name,CA SubCategory Name,French SubCategory Name,SubCategory CID,Division Slug Name,Division Slug Path,Category Slug Name,Category Slug Path,SubCategory Slug Name,SubCategory Slug Path,CA Division Slug Name,CA Division Slug Path,CA Category Slug Name,CA Category Slug Path,CA SubCategory Slug Name,CA SubCategory Slug Path,French Division Slug Name,French Division Slug Path,French Category Slug Name,French Category Slug Path,French SubCategory Slug Name,French SubCategory Slug Path,Object Type,Brand,Start Date,End Date,Hidden,Assortment Type,Old Division Slug Path,CA Old Division Slug Path,French Old Division Slug Path,Old Category Slug Path,CA Old Category Slug Path,French Old Category Slug Path,Old SubCategory Slug Path,CA Old SubCategory Slug Path,French Old SubCategory Slug Path,Division Redirect URL,CA Division Redirect URL,French Division Redirect URL,Category Redirect URL,CA Category Redirect URL,French Category Redirect URL,SubCategory Redirect URL,CA SubCategory Redirect URL,French SubCategory Redirect URL,Division SEO Meta Description,Division CA SEO Meta Description,Division French SEO Meta Description,Category SEO Meta Description,"+
	"Category CA SEO Meta Description,Category French SEO Meta Description,SubCategory SEO Meta Description,SubCategory CA SEO Meta Description,SubCategory French SEO Meta Description,"+	
	"Division SEO Page Title,Division CA SEO Page Title,Division French SEO Page Title,Category SEO Page Title,Category CA SEO Page Title,Category French SEO Page Title,SubCategory SEO Page Title,SubCategory CA SEO Page Title,SubCategory French SEO Page Title,"+
	"Division SEO Page Index Tag,Division CA SEO Page Index Tag,Category SEO Page Index Tag,Category CA SEO Page Index Tag,SubCategory SEO Page Index Tag,SubCategory CA SEO Page Index Tag"+"\n");
...
function getContextSpecificValues(object, sAttribute, context)
	{
		...
		contextSpecificAttrValue = stepManager.executeInContext(context, function(specificManager) {
				...
				if (isCategoryInActive(ObjectinContext) ==  true ){
...
//################################################################
	...
	//################################################################
function isCategoryInActive(categoryOBj){
	...
	var description = categoryOBj.getValue("a_Category_Description").getSimpleValue();	
		if(description != null )
		{
			...
			var onedayDate = new Date(today);
			onedayDate.setDate(today.getDate() - 1);
			var formattedEndDate =  new Date(endDate)  ;
			
				// check if the end date is more than the date of today +8 
			if( (formattedEndDate != null)  && (formattedEndDate > pastDate) && (formattedEndDate < onedayDate)){ return true ;}
			else{return false ;}
		}
	}
}
...
function getAttributeValueAsPerContext(objectNode, attribute, context){
			...
			asignValue = getContextSpecificValues(objectNode,attribute,"EN_CA");
			}
			return asignValue;
}
...
//################################################################
//*********check If a Division/Category/Subcategory are exclusive in Canada Market***********
//################################################################	
function isCategoryInActiveWithContext(hierarchyNode,context)
{
		var cxtSpeciDivObj = getContextSpecificObject(hierarchyNode, context);
		var isInActiveDivision= "" ;
		if(cxtSpeciDivObj != "undefined" )
		{
		isInActiveDivision = isCategoryInActive(cxtSpeciDivObj) ;
		}
		...
		if(context == "EN_CA" && isInActiveDivision == true ){
			...
			isInActiveDivision = false ;
			}else{
			// if its active only in EN_CA and not in EN_US market 
			isInActiveDivision = true ;
			}
		}			
		return isInActiveDivision ;		
}
...
{		//Division ID
			...
			if(isCategoryInActiveWithContext(divisiondObj,divContext) == true ){
			...
			var ctyStartDate = getAttributeValueAsPerContext(divisiondObj, "a_WebCategory_Start_Date", divContext);	
			//var ctyEndDate =getAttributeValue(divisiondObj,"a_WebCategory_End_Date");
			var ctyEndDate = getAttributeValueAsPerContext(divisiondObj, "a_WebCategory_End_Date", divContext);	
			...
			var seoMetadataDescription = getAttributeValueAsPerContext(divisiondObj, "a_SEO_Meta_Description", divContext);
			var en_CA_seoMetadataDescription= getAttributeValueForOtherContext(divisiondObj, "a_SEO_Meta_Description", divContext);
			var fr_CA_seoMetadataDescription =  getContextSpecificValues(divisiondObj,"a_SEO_Meta_Description","FR_CA");

			//SEO Page Title
			var seoPageTitle = getAttributeValueAsPerContext(divisiondObj, "a_SEO_Page_Title", divContext);
			var en_CA_seoPageTitle= getAttributeValueForOtherContext(divisiondObj, "a_SEO_Page_Title", divContext);
			var fr_CA_seoPageTitle =  getContextSpecificValues(divisiondObj,"a_SEO_Page_Title","FR_CA");

			//Indexable
			var Indexable = getAttributeValueAsPerContext(divisiondObj, "a_SEO_Non_Indexable", divContext);
			var en_CA_Indexable= getAttributeValueForOtherContext(divisiondObj, "a_SEO_Non_Indexable", divContext);
			
			fw.write( divisionId+","+divCtyName+","+en_CA_divCtyName+","+fr_CA_divCtyName +","+divisionCID+","+","+","+","+","+","+","+","+","+divisionSlug+","+categoryNLU+","+","+","+","+","+en_CA_divslugName+","+en_CA_divslugNLU+","+","+","+","+","+fr_CA_divslugName+","+fr_CA_divslugNLU+","+","+","+","+","+objectTypeName+","+brandNo+","+ctyStartDate+","+ctyEndDate+","+categoryHide+","+assortmentType+","+oldDivisionSlug+","+en_CA_OlddivSlug+","+fr_CA_oldDivSlug+","+divRedirectUrl+","+en_CA_divRedirectUrl+","+fr_CA_divRedirectUrl+
			+","+","+","+","+","+","+","+seoMetadataDescription+","+en_CA_seoMetadataDescription+","+fr_CA_seoMetadataDescription+","+","+","+","+","+","+","+
			seoPageTitle+","+en_CA_seoPageTitle+","+fr_CA_seoPageTitle+","+","+","+","+","+","+","+Indexable+","+en_CA_Indexable+"\n");
		}	
}
	//################################################################
	...
	{	
		if(isCategoryInActiveWithContext(ctyObject,ctyContext) == true ){
			//Category ID 
			...
			var ctyCtyStartDate = getAttributeValueAsPerContext(ctyObject, "a_WebCategory_Start_Date", ctyContext);
			//var ctyCtyEndDate =getAttributeValue(ctyObject,"a_WebCategory_End_Date");
			var ctyCtyEndDate = getAttributeValueAsPerContext(ctyObject, "a_WebCategory_End_Date", ctyContext);
			...
			// for redirect url 
			var ctyDiviReDirectUrl = getAttributeValueAsPerContext(divObject, "a_Redirect_URL", ctyContext);
			
			var en_CA_ctyDiviReDirectUrl =  getAttributeValueForOtherContext(divObject, "a_Redirect_URL", ctyContext);
		
			var fr_CA_ctyDiviReDirectUrl =  getContextSpecificValues(divObject,"a_Redirect_URL","FR_CA");
						
			var ctyReDirectUrl = getAttributeValueAsPerContext(ctyObject, "a_Redirect_URL", ctyContext);
			
			var en_CA_ctyReDirectUrl =  getAttributeValueForOtherContext(ctyObject, "a_Redirect_URL", ctyContext);
			
			var fr_CA_ctyReDirectUrl =  getContextSpecificValues(ctyObject,"a_Redirect_URL","FR_CA");
			// End Redirect URL


			var seoMetadataDescription = getAttributeValueAsPerContext(ctyObject, "a_SEO_Meta_Description", ctyContext);
			var en_CA_seoMetadataDescription= getAttributeValueForOtherContext(ctyObject, "a_SEO_Meta_Description", ctyContext);
			var fr_CA_seoMetadataDescription =  getContextSpecificValues(ctyObject,"a_SEO_Meta_Description","FR_CA");
			
			// Division SEO Metadata
			var div_seoMetadataDescription = getAttributeValueAsPerContext(divObject, "a_SEO_Meta_Description", ctyContext);
			var div_en_CA_seoMetadataDescription= getAttributeValueForOtherContext(divObject, "a_SEO_Meta_Description", ctyContext);
			var div_fr_CA_seoMetadataDescription =  getContextSpecificValues(divObject,"a_SEO_Meta_Description","FR_CA");


			//Category SEO Page Title
			var seoPageTitle = getAttributeValueAsPerContext(ctyObject, "a_SEO_Page_Title", ctyContext);
			var en_CA_seoPageTitle= getAttributeValueForOtherContext(ctyObject, "a_SEO_Page_Title", ctyContext);
			var fr_CA_seoPageTitle =  getContextSpecificValues(ctyObject,"a_SEO_Page_Title","FR_CA");


			//Division SEO Page Title
			var div_seoPageTitle = getAttributeValueAsPerContext(divObject, "a_SEO_Page_Title", ctyContext);
			var div_en_CA_seoPageTitle= getAttributeValueForOtherContext(divObject, "a_SEO_Page_Title", ctyContext);
			var div_fr_CA_seoPageTitle =  getContextSpecificValues(divObject,"a_SEO_Page_Title","FR_CA");
			

			//Indexable
			var Indexable = getAttributeValueAsPerContext(ctyObject, "a_SEO_Non_Indexable", ctyContext);
			var en_CA_Indexable= getAttributeValueForOtherContext(ctyObject, "a_SEO_Non_Indexable", ctyContext);

			//Division Indexable
			var div_Indexable = getAttributeValueAsPerContext(divObject, "a_SEO_Non_Indexable", ctyContext);
			var div_en_CA_Indexable= getAttributeValueForOtherContext(divObject, "a_SEO_Non_Indexable", ctyContext);
			
		fw.write( ctyId+","+ctyDivName+","+en_CA_ctyDivName+","+fr_CA_ctyDivName +","+ctyDivCID+","+ctyCtyName+","+en_CA_CtyName+","+fr_CA_CtyName+","+ctyCID+","+","+","+","+","+ctyDivisionSlug+","+ctyDivNLU+","+ctySlug+","+ctyNLU+","+","+","+en_CA_divSlug+","+en_CA_divNLU+","+en_CA_ctySlugName+","+en_CA_ctySlugNLU+","+","+","+fr_CA_divSlug+","+fr_CA_divNLU+","+fr_CA_ctySlugName+","+fr_CA_ctySlugNLU+","+","+","+objectTypeCty+","+ctyBrandNo+","+ctyCtyStartDate+","+ctyCtyEndDate+","+ctyCategoryHide+","+ctyAssortmentType+","+ctyOldDivisionSlug+","+en_CA_ctyOlddivSlug+","+fr_CA_ctyOldDivSlug+","+ctyOldSlug+","+en_CA_ctyOldSlug+","+fr_CA_ctyOldSlug+",,,,"+ctyDiviReDirectUrl+","+en_CA_ctyDiviReDirectUrl+","+fr_CA_ctyDiviReDirectUrl+","+ctyReDirectUrl+","+en_CA_ctyReDirectUrl+","+fr_CA_ctyReDirectUrl+
				",,,,"+div_seoMetadataDescription+","+div_en_CA_seoMetadataDescription+","+div_fr_CA_seoMetadataDescription+","+seoMetadataDescription+","+en_CA_seoMetadataDescription+","+fr_CA_seoMetadataDescription+","+","+","+","+
		div_seoPageTitle+","+div_en_CA_seoPageTitle+","+div_fr_CA_seoPageTitle+","+seoPageTitle+","+en_CA_seoPageTitle+","+fr_CA_seoPageTitle+",,,,"+div_Indexable+","+div_en_CA_Indexable+","+Indexable+","+en_CA_Indexable+"\n");
		}	
	}
	//################################################################
			//********** write Category details  ***********
	...
	{			
			if(isCategoryInActiveWithContext(subCtyObject,subCtyContext) == true ){
			...
			var subCtyStartDate = getAttributeValueAsPerContext(subCtyObject, "a_WebCategory_Start_Date", subCtyContext);
			//var subCtyEndDate =getAttributeValue(subCtyObject,"a_WebCategory_End_Date");
			var subCtyEndDate = getAttributeValueAsPerContext(subCtyObject, "a_WebCategory_End_Date", subCtyContext);
			...
			// for redirect url 
			var subCtyDiviReDUrl = getAttributeValueAsPerContext(subctyDivObject, "a_Redirect_URL", subCtyContext);
			var en_CA_subCtyDiviReDUrl =  getAttributeValueForOtherContext(subctyDivObject, "a_Redirect_URL", subCtyContext);
		
			var fr_CA_subCtyDiviReDUrl =  getContextSpecificValues(subctyDivObject,"a_Redirect_URL","FR_CA");
						
			var ctyReDUrl = getAttributeValueAsPerContext(subctyCtyObject, "a_Redirect_URL", subCtyContext);
			
			var en_CA_ctyReDUrl =  getAttributeValueForOtherContext(subctyCtyObject, "a_Redirect_URL", subCtyContext);
			
			var fr_CA_ctyReDUrl =  getContextSpecificValues(subctyCtyObject,"a_Redirect_URL","FR_CA");
			
			var subCtyReDirectUrl = getAttributeValueAsPerContext(subCtyObject, "a_Redirect_URL", subCtyContext);
			
			var en_CA_subCtyReDirectUrl =  getAttributeValueForOtherContext(subCtyObject, "a_Redirect_URL", subCtyContext);
			
			var fr_CA_subCtyReDirectUrl =  getContextSpecificValues(subCtyObject,"a_Redirect_URL","FR_CA");
			// End Redirect URL

			var seoMetadataDescription = getAttributeValueAsPerContext(subCtyObject, "a_SEO_Meta_Description", subCtyContext);
			var en_CA_seoMetadataDescription= getAttributeValueForOtherContext(subCtyObject, "a_SEO_Meta_Description", subCtyContext);
			var fr_CA_seoMetadataDescription =  getContextSpecificValues(subCtyObject,"a_SEO_Meta_Description","FR_CA");

			//division seo metadata description
			var div_seoMetadataDescription = getAttributeValueAsPerContext(subctyDivObject, "a_SEO_Meta_Description", subCtyContext);
			var div_en_CA_seoMetadataDescription= getAttributeValueForOtherContext(subctyDivObject, "a_SEO_Meta_Description", subCtyContext);
			var div_fr_CA_seoMetadataDescription =  getContextSpecificValues(subctyDivObject,"a_SEO_Meta_Description","FR_CA");

			//Category seo metadata description
			var cat_seoMetadataDescription = getAttributeValueAsPerContext(subctyCtyObject, "a_SEO_Meta_Description", subCtyContext);
			var cat_en_CA_seoMetadataDescription= getAttributeValueForOtherContext(subctyCtyObject, "a_SEO_Meta_Description", subCtyContext);
			var cat_fr_CA_seoMetadataDescription =  getContextSpecificValues(subctyCtyObject,"a_SEO_Meta_Description","FR_CA");

			
		     //SubCategory SEO Page Title
			var seoPageTitle = getAttributeValueAsPerContext(subCtyObject, "a_SEO_Page_Title", subCtyContext);
			var en_CA_seoPageTitle= getAttributeValueForOtherContext(subCtyObject, "a_SEO_Page_Title", subCtyContext);
			var fr_CA_seoPageTitle =  getContextSpecificValues(subCtyObject,"a_SEO_Page_Title","FR_CA");
			
			//Category SEO Page Title
			var cat_seoPageTitle = getAttributeValueAsPerContext(subctyCtyObject, "a_SEO_Page_Title", subCtyContext);
			var cat_en_CA_seoPageTitle= getAttributeValueForOtherContext(subctyCtyObject, "a_SEO_Page_Title", subCtyContext);
			var cat_fr_CA_seoPageTitle =  getContextSpecificValues(subctyCtyObject,"a_SEO_Page_Title","FR_CA");


			//Division SEO Page Title
			var div_seoPageTitle = getAttributeValueAsPerContext(subctyDivObject, "a_SEO_Page_Title", subCtyContext);
			var div_en_CA_seoPageTitle= getAttributeValueForOtherContext(subctyDivObject, "a_SEO_Page_Title", subCtyContext);
			var div_fr_CA_seoPageTitle =  getContextSpecificValues(subctyDivObject,"a_SEO_Page_Title","FR_CA");
			

			//Category Indexable
			var cat_Indexable = getAttributeValueAsPerContext(subctyCtyObject, "a_SEO_Non_Indexable", subCtyContext);
			var cat_en_CA_Indexable= getAttributeValueForOtherContext(subctyCtyObject, "a_SEO_Non_Indexable", subCtyContext);
			
			//Sub Categeory Indexable
			var Indexable = getAttributeValueAsPerContext(subCtyObject, "a_SEO_Non_Indexable", subCtyContext);
			var en_CA_Indexable= getAttributeValueForOtherContext(subCtyObject, "a_SEO_Non_Indexable", subCtyContext);

			//Division Indexable
			var div_Indexable = getAttributeValueAsPerContext(subctyDivObject, "a_SEO_Non_Indexable", subCtyContext);
			var div_en_CA_Indexable= getAttributeValueForOtherContext(subctyDivObject, "a_SEO_Non_Indexable", subCtyContext);

		fw.write( subCtyid+","+subctyDivName+","+en_CA_subCtyDivName+","+fr_CA_subCtyDivName +","+subCtyDivCID+","+subCtyCtyName+","+en_CA_subCtyCtyName+","+fr_CA_subCtyCtyName+","+subCtyCtyCID+","+subCtyName+","+en_CA_subCtyName+","+fr_CA_subCtyName+","+subCtyCID+","+SubctyDivisionSlug+","+subCtyDivNLU+","+subCtyCtySlug+","+subCtyCtyNLU+","+subCtySlug+","+subCtyNLU+","+en_CA_subCtyDivSlug+","+en_CA_subCtyDivNLU+","+en_CA_subCtyCtySlugName+","+en_CA_subCtyCtySlugNLU+","+en_CA_subCtySlugName+","+en_CA_subCtySlugNLU+","+fr_CA_subCtyDivSlug+","+fr_CA_subCtyDivNLU+","+fr_CA_subCtyCtySlugName+","+fr_CA_subCtyCtyNLU+","+fr_CA_subCtySlugName+","+fr_CA_subCtySlugNLU+","+objectTypeSubCty+","+subCtyBrandNo+","+subCtyStartDate+","+subCtyEndDate+","+subCategoryHide+","+subCtyAssortmentType+","+subCtyOldDivisionSlug+","+en_CA_subCtyOldDivisionSlug+","+fr_CA_subCtyOldDivisionSlug+","+subCtyctyOldSlug+","+en_CA_subCtyctyOldSlug+","+fr_CA_subCtyctyOldSlug+","+subCtyOldSlug+","+en_CA_subCtyOldSlug+","+fr_CA_subCtyOldSlug+","+subCtyDiviReDUrl+","+en_CA_subCtyDiviReDUrl+","+fr_CA_subCtyDiviReDUrl+","+ctyReDUrl+","+en_CA_ctyReDUrl+","+fr_CA_ctyReDUrl+","+subCtyReDirectUrl+","+en_CA_subCtyReDirectUrl+","+fr_CA_subCtyReDirectUrl+
		","+div_seoMetadataDescription+","+div_en_CA_seoMetadataDescription+","+div_fr_CA_seoMetadataDescription+","+cat_seoMetadataDescription+","+cat_en_CA_seoMetadataDescription+","+cat_fr_CA_seoMetadataDescription+","+seoMetadataDescription+","+en_CA_seoMetadataDescription+","+fr_CA_seoMetadataDescription+","+
				div_seoPageTitle+","+div_en_CA_seoPageTitle+","+div_fr_CA_seoPageTitle+","+cat_seoPageTitle+","+cat_en_CA_seoPageTitle+","+cat_fr_CA_seoPageTitle+","+seoPageTitle+","+en_CA_seoPageTitle+","+fr_CA_seoPageTitle+","+
				div_Indexable+","+div_en_CA_Indexable+","+cat_Indexable+","+cat_en_CA_Indexable+","+Indexable+","+en_CA_Indexable+"\n");
		}	
	}
//################################################################
			//********** Main block ************
...
function mainMethod(context){
		...
		for(var brandId = 0 ; brandId<brandList.size();brandId ++)
		{
			...
			for(var divId = 0 ; divId<divisionList.size();divId ++)
			{
				   var divisiondObj= divisionList.get(divId);
					getDivisionDetails(divisiondObj,context);
					//returns the categories under the division 
					var ctyList  =  divisiondObj.getChildren();
					for(var ctyListId = 0 ; ctyListId<ctyList.size();ctyListId ++)
					{
						var ctyObject= ctyList.get(ctyListId);
						getCategoryDetails(ctyObject,context);
						// returns sub categories under the category 
						var subCtyList  =  ctyObject.getChildren();
	...
	//var emailIDTO  = mailMethod.addTo("sai_preethi_mandipalli@gap.com;jagadish_beejapu@gap.com;sri_indu_dekkapati@gap.com;");
	var emailIDTO  = mailMethod.addTo("natalie_bargas@gap.com;merry_touch@gap.com;janette_fuentes@gap.com;alexander_bocknek@gap.com;sai_preethi_mandipalli@gap.com;jagadish_beejapu@gap.com;sri_indu_dekkapati@gap.com;rohith_kasula@gap.com;aswathi_naranath@gap.com;priyadarshini_sahoo@gap.com;HQ-SEO-DG@gap.com;venugopal_nandimandalam@gap.com");
	var emailSubject  = mailMethod.subject("SEO Inactive Divisions,Categories and Subcategories report in Past 7 days");
	var emailBody  = mailMethod.plainMessage("Please find SEO Report for inactive Divisions,Categories and Subcategories in the past seven days (7days ) across all the contexts i.e EN_US , EN_CA, FR_CA \n if you have any questions please reach out to support team thanks ");
	// set attachment 
	var attachment = mailMethod.attachment();
	var fromAsssest = attachment.fromAsset(asset);
	var setAttachMentName  =  attachment.name("SEO-PIM-Report_For_Inactive_Categories_in_Past7Days.csv");
	attachment.attach();
	//send email 
