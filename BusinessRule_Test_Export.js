...
function mainMethod(context) {
	var brand_BRFS = stepManager.getClassificationHome().getClassificationByID("WBU-1038093");
	var divisionList = brand_BRFS.getChildren();
	...
	for (var i = 0; i < divisionList.size(); i++) {
		...
		if (divObj != null) {
			...
			if (objectType == "WebDivision") {
				...
				if (isActiveDiv == true) {
					getDivisionDetails(divObj, context);
				}
			}
		}
	}
		...
		if (subCatObj != null) {
			...
			if (objectType == "WebSubCategory") {
				...
				if (isActiveSubCat == true) {
					getSubCategoryDetails(subCatObj, context);
				}
			}
		}
	}
...
var emailIDTO = mailMethod.addTo("jagadish_beejapu@gap.com;sri_indu_dekkapati@gap.com;venugopal_nandimandalam@gap.com;");
var emailSubject = mailMethod.subject("SEO preview NLU's ");
var emailBody = mailMethod.plainMessage("Please find BRFS NLU Preview URL's for US ");
