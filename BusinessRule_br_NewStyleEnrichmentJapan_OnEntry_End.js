...
{
  "id" : "br_NewStyleEnrichmentJapan_OnEntry_End",
  "type" : "BusinessAction",
  "setupGroups" : [ "Workflows" ],
  "name" : "NewStyleEnrichmentJapan_OnEntry_End",
  "description" : null,
  "scope" : "Global",
  "validObjectTypes" : [ ],
...
if (node.getValue("a_Translation_Status").getSimpleValue() == "Complete") {
	...
	step.executeInContext('JA_JP',function(frContextManager) {
...
for(var i = 0; i < CCList.size(); i++) {
	var cc = CCList.get(i);
	if(cc.isInState("wf_CCEnrichmentJapan","NewCCEnrich_Final")) {
		cc.getTaskByID("wf_CCEnrichmentJapan","NewCCEnrich_Final").triggerByID("Submit", "Auto-approving CC from New Style Enrichment workflow");
	}
}
}
/*===== business rule plugin definition =====
