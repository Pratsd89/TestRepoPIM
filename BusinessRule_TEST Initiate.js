...
/*===== business rule definition =====
{
  ...
  "name" : "TEST InitiateWorkflowAction",
  "description" : null,
  "scope" : "Global",
...
function CopyShots(fromContext, market) {
    step.executeInContext(fromContext, function (contextManager) {
        ...
        for (var i = 0; i < refs.size(); i++) {
            ...
            if (shot.getValue("a_Shot_Request_Method").getSimpleValue() == "ASLR" && fromContext == "EN_US") {
            ...
            if (shared != null && shared.contains(market) && (!(shared.contains(currentMarket)))) {
...
function CopyOverviewBullets(fromContext) {
    step.executeInContext(fromContext, function (contextManager) {
        var currentProduct = contextManager.getProductHome().getProductByID(node.getID());
    ...
    if (objectType == "Style") {
        ...
        if (styleMktDsg == null) {
            ...
            if (currentMarket == "SA") {
                ...
                if (bomStyleRefs.size() > 0) {
                    for (var i = 0; i < bomStyleRefs.size(); i++) {
                        ...
                        if (ref_bomStyle) {
                            ...
                            if (inheritOption == null || !inheritOption.contains("SA")) {
                ...
                else if (styleMktDsg.contains("US")) {
                    var inheritOption = node.getValue("a_Inherit_US_Copy_Option").getSimpleValue();
                    ...
                    if (inheritOption != null) {
                        if (styleMktDsg.contains(currentMarket) && inheritOption.contains(currentMarket)) {
                            CopyOverviewBullets("EN_US");
                        }
                    }
                }
                else if (styleMktDsg.contains("JPN")) {
                    var inheritOption = node.getValue("a_Inherit_US_Copy_Option").getSimpleValue();
                    if (inheritOption == null) {
                        node.getValue("a_Inherit_US_Copy_Option").setSimpleValue(currentMarket);
                        inheritOption = node.getValue("a_Inherit_US_Copy_Option").getSimpleValue();
    ...
    if (objectType == "CustomerChoice") {
        if (currentMarket == "SA") {
            ...
            if (BOM_CCrefs.size() > 0) {
                ...
                else if (aclMarket == currentMarket && mktDsg.contains("JPN")) {
                    CopyShots("EN_JP", "JPN");
                }
            }
