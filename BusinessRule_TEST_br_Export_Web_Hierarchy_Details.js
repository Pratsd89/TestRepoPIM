...
{
  "id" : "TEST_br_Export_Web_Hierarchy_Details",
  "type" : "BusinessAction",
  "setupGroups" : [ "Actions" ],
  "name" : "TEST_br_Export_Web_Hierarchy_Details",
  "description" : null,
  "scope" : "Global",
  "validObjectTypes" : [ ],
...
exports.operation0 = function (stepManager,node,mail) {
	...
	var loggerfilePath = "/opt/stibo/SEO-PIM-Report_For_Active_And_Upcomming_Categories_logger.csv";
	var loggerfile = new java.io.File(loggerfilePath);
	...
	var loggerfw= new java.io.FileWriter(loggerfile,false);
...
function mainMethod(context)
{
					...
					if(isActiveDivision == true  )
					{
						...
						//returns the category under the division Unblock this after testing divisions 
						 ...
						 for(var ctyListId = 0 ; ctyListId<ctyList.size();ctyListId ++)
						 {
								...
								loggerfw.write("Main Cat" + ctyObject.getID() + "\n");
								...
								if(isActiveCty == true  )
								{
									...
									loggerfw.write(isActiveCty + "Main2 Cat" + ctyObject.getID() + "\n");
	...
	var loggerfileInputStream = new java.io.FileInputStream(loggerfile);
	var asset = stepManager.getAssetHome().getAssetByID("TF_128425972");
	var loggerasset = stepManager.getAssetHome().getAssetByID("TF_128425974");
	var uploaded = asset.upload(fileInputStream,filePath);
	var uploaded1 = loggerasset.upload(loggerfileInputStream,loggerfilePath);
	// set up Email 
	...
	var fromAsssest1 = attachment.fromAsset(loggerasset);
