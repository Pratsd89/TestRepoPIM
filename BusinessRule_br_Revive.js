...
/*===== business rule definition =====
{
  ...
  "type" : "BusinessAction",
  "setupGroups" : [ "Actions" ],
  "name" : "Revive Web Category(DELETE)",
  "description" : null,
  "scope" : "Global",
...
*/
exports.operation0 = function (node,step,LKT,webUI,queryHome,Natural_Language_URL) {
        ...
        if (archiveNluForCty != null) {
            ...
            if (oldParentIDOfWebCty != null) {
                ...
                if (isCategoryExists != 0) {
                    ...
                    var cntxtNode = step.getClassificationHome().getClassificationByID(node.getID());
                    ...
                    for (var j = 0; j < childernSubCategories.length; j++) {
                        ...
                        var CanInheritOption = childernSubCategories[j].getValue("a_CAN_Inherit_Option").getSimpleValue();
                        if (CanInheritOption != null) {
                            removeCASpecificValues(childernSubCategories[j]);
                        }
                    }
        ...
        if (archiveNluForDivision != null) {
            ...
            if (isDivisionExists != 0) {
                ...
                for (var ctyIndex = 0; ctyIndex < childernCategories.length; ctyIndex++) {
                    ...
                    var CanInheritOption = childernCategories[ctyIndex].getValue("a_CAN_Inherit_Option").getSimpleValue();
                    if (CanInheritOption != null) {
                        removeCASpecificValues(childernCategories[ctyIndex]);
                    }
                    ...
                    for (var subCtyIndex = 0; subCtyIndex < subCtyNode.length; subCtyIndex++) {
                        ...
                        var CanInheritOption = subCtyNode[subCtyIndex].getValue("a_CAN_Inherit_Option").getSimpleValue();
                        if (CanInheritOption != null) {
                            removeCASpecificValues(subCtyNode[subCtyIndex]);
                        }
                    }
                }
...
function removeCASpecificValues(node) {
    step.executeInContext("EN_CA", function(currentContextManager) {
        var Classification = currentContextManager.getClassificationHome().getClassificationByID(node.getID());
        Classification.getValue("a_Category_Description").setValue(null);
        Classification.getValue("a_Natural_Language_URL").setValue(null);
        Classification.getValue("a_Category_Slug").setValue(null);
    });
}
}