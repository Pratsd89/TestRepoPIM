...
{
  "id" : "br_Auto_Trigger_SA_WF_BOM_CC",
  "type" : "BusinessAction",
  "setupGroups" : [ "Actions" ],
  "name" : "Auto Trigger SA WF When US BOM CC Complete",
  "description" : null,
  "scope" : "Global",
	...
	for (var i = 0; i < ccRefs.size(); i++) {
		if (ccRefs.get(i).getReferenceType().getID() == "rt_BOM_CC") {
			...
			stepManager.executeInContext("EN_US", function(enContextManager) {
				...
				if (usLifeCycleStatus == 'Approved' || (usLifeCycleStatus=='In Progress' && usCCPhotoStatus =='Complete')) {
					autoTrigger = true;
				}
