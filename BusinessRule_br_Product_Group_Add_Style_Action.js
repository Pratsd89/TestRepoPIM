...
{
  "id" : "br_Product_Group_Add_Style_Action",
  "type" : "BusinessAction",
  "setupGroups" : [ "Super PDP Consolidation" ],
  "name" : "Product Group Add Style Action",
...
function utilityMethod(styleList, index, primaryStyleID) {
    ...
    for (var i = index; i < styleList.length; i++) {
        ...
        if (currentStyle == null) {
            ...
            var currentStyleBrand = currentStyleClass.getValue("a_Brand_Number").getSimpleValue();
            var parentStyleBrand = parentClass.getValue("a_Brand_Number").getSimpleValue();
            if ((parentClass.getID() == currentStyleClass.getID()) || (parentStyleBrand=='GPS' &&currentStyleBrand=='GPS') )  {
                var flag = false;
