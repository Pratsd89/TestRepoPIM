...
{
  "id" : "Test_InitiateWorkflowAction",
  "type" : "BusinessAction",
  "setupGroups" : [ "Actions" ],
  "name" : "Test_InitiateWorkflowAction",
  "description" : null,
  "scope" : "Global",
...
if (currentContext != "FR_CA" && currentContext != "JA_JP") {
	...
	if (objectType == "Style") {
		...
		if (styleMktDsg == null) {
			...
			if (currentMarket != "US" && styleMktDsg.contains("US")) {
			...
			else if (currentMarket == "SA" && styleMktDsg.contains("JPN") && !styleMktDsg.contains("US")){
				var inheritJPOption = node.getValue("a_Inherit_JPN_Copy_Option").getSimpleValue();
				if (inheritJPOption == null) {
					node.getValue("a_Inherit_JPN_Copy_Option").setSimpleValue(currentMarket);
					inheritJPOption = node.getValue("a_Inherit_JPN_Copy_Option").getSimpleValue();
				}
				
				if (inheritJPOption != null) {
					if (styleMktDsg.contains(currentMarket) && inheritJPOption.contains(currentMarket)) {
						step.executeInContext('EN_JP', function (jpContextManager) {
							...
							var attributeGroup = jpContextManager.getAttributeGroupHome().getAttributeGroupByID('ag_English_Replication_Attributes');
							var attributeList = attributeGroup.getAttributes().toArray();

							for (var y = 0; y < attributeList.length; y++) {
								var jpAttributeValue = jpCurrentProduct.getValue(attributeList[y].getID()).getSimpleValue();

								step.executeInContext(currentContext, function (otherContextManager) {
									var otherCurrentProduct = otherContextManager.getProductHome().getProductByID(node.getID());
									otherCurrentProduct.getValue(attributeList[y].getID()).setSimpleValue(jpAttributeValue);
								});
							}
						});
					}
			...
			else if(currentMarket == "JPN" && styleMktDsg.contains("SA") && !styleMktDsg.contains("US")){
				var inheritJPOption = node.getValue("a_Inherit_JPN_Copy_Option").getSimpleValue();
				step.executeInContext("EN_SA", function (otherManager) {
					...
					if ((copyStatus == "In Progress" || copyStatus == "Complete") && lifeCycleStatus != "Approved"){
						if (inheritJPOption == null) {
							node.getValue("a_Inherit_JPN_Copy_Option").setSimpleValue("SA");
						}
						inheritJPOption = node.getValue("a_Inherit_JPN_Copy_Option").getSimpleValue();
					}
				});
