/*===== export metadata =====
{
  "contextId" : "EN_CA",
  "workspaceId" : "Main"
}
*/
/*===== business rule definition =====
{
  "id" : "br_Filter_Invalid_PhotShot_Events",
  "type" : "BusinessCondition",
  "setupGroups" : [ "Conditions" ],
  "name" : "br_Filter_Invalid_PhotShot_Events",
  "description" : null,
  "scope" : "Global",
  "validObjectTypes" : [ "ProductShotRequest" ],
  "allObjectTypesValid" : false,
  "runPrivileged" : false,
  "onApprove" : "Never",
  "dependencies" : [ ]
}
*/
/*===== business rule plugin definition =====
{
  "pluginId" : "JavaScriptBusinessConditionWithBinds",
  "binds" : [ {
    "contract" : "CurrentEventTypeBinding",
    "alias" : "currentEvent",
    "parameterClass" : "null",
    "value" : null,
    "description" : null
  }, {
    "contract" : "LoggerBindContract",
    "alias" : "logger",
    "parameterClass" : "null",
    "value" : null,
    "description" : null
  } ],
  "messages" : [ ],
  "pluginType" : "Operation"
}
*/
exports.operation0 = function (currentEvent,logger) {
// Trigger OIEP only for Valid Shots
logger.info("Executing the AssetHub US Outbound Filter Condition. Event: "+ currentEvent.getID() );
if (currentEvent.getID() == "Create"){
	return false;	
	}
else{   	
	return true;
	}

}
/*===== business rule plugin definition =====
{
  "pluginId" : "JavaScriptBusinessConditionWithBinds",
  "binds" : [ {
    "contract" : "CurrentObjectBindContract",
    "alias" : "node",
    "parameterClass" : "null",
    "value" : null,
    "description" : null
  }, {
    "contract" : "ManagerBindContract",
    "alias" : "step",
    "parameterClass" : "null",
    "value" : null,
    "description" : null
  }, {
    "contract" : "LoggerBindContract",
    "alias" : "log",
    "parameterClass" : "null",
    "value" : null,
    "description" : null
  } ],
  "messages" : [ ],
  "pluginType" : "Operation"
}
*/
exports.operation1 = function (node,step,log) {
//The below functionaliy is implemented as part of Dropship Requirements-https://gapinc.atlassian.net/browse/PPIM-8317
var result = true;
step.executeInContext('EN_US',function(usContextManager){
	var usCurrentEntity = usContextManager.getEntityHome().getEntityByID(node.getID());
	log.info(usCurrentEntity.getValue("a_Shot_Brand_Number").getSimpleValue());
	log.info(usCurrentEntity.getValue("a_Shot_Channel_Number").getSimpleValue());
	log.info(usCurrentEntity.getValue("a_Shot_Universal_Style_Number").getSimpleValue());
	log.info(usCurrentEntity.getValue("a_Shot_Universal_CC_Number").getSimpleValue());
	
	if (usCurrentEntity.getValue("a_Shot_Universal_Style_Number").getSimpleValue() == null ||usCurrentEntity.getValue("a_Shot_Universal_Style_Number").getSimpleValue()==''|| usCurrentEntity.getValue("a_Shot_Universal_CC_Number").getSimpleValue() == null|| usCurrentEntity.getValue("a_Shot_Universal_CC_Number").getSimpleValue()==''){
		result = false;
		}
	});
	
return result;
}
/*===== business rule plugin definition =====
{
  "pluginId" : "JavaScriptBusinessConditionWithBinds",
  "binds" : [ {
    "contract" : "CurrentObjectBindContract",
    "alias" : "node",
    "parameterClass" : "null",
    "value" : null,
    "description" : null
  }, {
    "contract" : "LoggerBindContract",
    "alias" : "log",
    "parameterClass" : "null",
    "value" : null,
    "description" : null
  }, {
    "contract" : "ManagerBindContract",
    "alias" : "step",
    "parameterClass" : "null",
    "value" : null,
    "description" : null
  } ],
  "messages" : [ ],
  "pluginType" : "Operation"
}
*/
exports.operation2 = function (node,log,step) {
//PPIM-7078
var result = true;
//PPIM-8552
var time = new java.util.Date();
var iso = new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
step.executeInContext('EN_US', function (usContextManager) {
	var usCurrentEntity = usContextManager.getEntityHome().getEntityByID(node.getID());
	//log.info(usCurrentEntity);
	//log.info(usCurrentEntity.getValue("a_Site_Placement").getSimpleValue()+usCurrentEntity.getValue("a_Shared_Markets").getSimpleValue()+usCurrentEntity.getValue("a_Shot_Code").getSimpleValue()+usCurrentEntity.getValue("a_Shot_CC_Number").getSimpleValue());
	if (usCurrentEntity.getValue("a_Site_Placement").getSimpleValue() == null || usCurrentEntity.getValue("a_Shared_Markets").getSimpleValue() == null || usCurrentEntity.getValue("a_Shot_Code").getSimpleValue() == null || usCurrentEntity.getValue("a_Shot_CC_Number").getSimpleValue() == null) {
		result = false;
	}
});
if (result == true) {
	node.getValue("a_File_AssetHub_TimeStamp").setSimpleValue(iso.format(time));
}
return result;
}