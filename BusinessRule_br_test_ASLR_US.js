...
*/
exports.operation0 = function (node,stepManager,CCToPhotoShotRef,PhotoShotRoot,PhotoShotObject,LKT,assethubqueueUS,assethubqueueCA,assethubqueueSA,eventType,mail,LIB_BRFS) {
...
fw.write("Brand,CCID,ASLRShotCode,ASLRShotType,ASLRSitePlacement,CCShotCode,CCShotType,CCSitePlacement,shotStatus\n");
...
function createShotRequest(shotCode, shotType, sitePlacement, CC) {
	log.info("creating Shot Request");
...
function checkShotExists(shotCode, shotType, sitePlacement, node2) {
	 var brand = node2.getValue('a_Brand_Number').getSimpleValue();
    var shots = node2.getReferences(CCToPhotoShotRef).toArray();
    for (var i = 0; i < shots.length; i++) {
        ...
        if (currShotCode == shotCode) {
            ...
            fw.write(brand + "," +node2.getID() +"," + shotCode + "," + shotType +"," + sitePlacement +","+ currShotCode + "," + currShotType +"," + currShotPlacement+","+currShotLcs+ "\n");
...
function checkASLREntry(currShot, node3) {
	log.info(currShot);
    var currShotCode = currShot.getValue("a_Shot_Code").getSimpleValue();
    var currShotType = currShot.getValue("a_Shot_Type").getSimpleValue();
    var currShotPlacement = currShot.getValue("a_Site_Placement").getSimpleValue();
...
var entityList = LIB_BRFS.getNodeList();

for (var z =0;z<entityList.size();z++){
   var node1 =  stepManager.getProductHome().getProductByID(entityList.get(z)); 
log.info(node1);

var marketCode = node1.getValue('a_Market_Designation').getSimpleValue();
var sortOrder = node1.getValue('a_CC_Sort_Order').getSimpleValue();
var ASLRDataContainers;
var subClass = node1.getParent().getParent();
...
var brandNum = node1.getValue('a_Brand_Number').getSimpleValue();
var brand_wpt = "a_" + brandNum + "_WebProductType";
var webProductType = node1.getValue(brand_wpt).getSimpleValue();
    ...
    if (marketCode.indexOf('US') != -1) {
        ...
        if (ASLRDataContainers != null) {
            for (var n = 0; n < ASLRDataContainers.length; n++) {
                ...
                var shotType = dataContainerObject.getValue('a_Shot_Type').getSimpleValue();
                var sitePlacement = dataContainerObject.getValue('a_ASLR_Site_Placement').getSimpleValue();
                if (!checkShotExists(shotCode, shotType, sitePlacement, node1)) {
                    //log.info("test");
                   // createShotRequest(shotCode, shotType, sitePlacement, node1); //if there is no shot which is in ASLR setting on the CC level, then the Shot is created at CC level
                   //fw.write(shotID + "," + shotCode + "," + shotType + "," + placement + "," + markets + "," + shotStatus + "," + shotCCNum + "," + shotUnivCCNum + "," + brand + "\n");
                  // fw.write(brand + "," node1 +"," + shotCode + "," + shotType +"," + sitePlacement);
                }
            }

           var shots = node1.getReferences(CCToPhotoShotRef).toArray();
            for (var i = 0; i < shots.length; i++) {
                var shot = shots[i].getTarget();
                if (!checkASLREntry(shot, node1)) {
                  //  var shotCancelStatus = cancelShots(shot);
                    //if (shotCancelStatus == "This Shot Request cannot be cancelled") {
                        ...
                        var shotCCNum = node1.getValue('a_CC_Number').getSimpleValue();
                        var shotUnivCCNum = node1.getID();
                        var brand = shot.getValue('a_Shot_Brand_Number').getSimpleValue();
                        fw.write(shotID + "," +node1.getID() +"," + shotCode + "," + shotType +"," + placement +","+ markets + "," + shotStatus +"," + shotCCNum+","+"this is generated difference"+ "\n");
                        //fw.write(shotID + "," + shotCode + "," + shotType + "," + placement + "," + markets + "," + shotStatus + "," + shotCCNum + "," + shotUnivCCNum + "," + brand + "\n");
                    //}
                }
            }
