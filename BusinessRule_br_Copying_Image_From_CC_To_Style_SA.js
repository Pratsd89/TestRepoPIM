...
{
  "id" : "br_Copying_Image_From_CC_To_Style_SA",
  "type" : "BusinessCondition",
  "setupGroups" : [ "Conditions" ],
  "name" : "Copying Image Ref From CC To Style SA Conditon",
  "description" : null,
  "scope" : "Global",
...
for (var i = 0; i < CCList.size(); i++) {
    ...
    if (sortOrder != null) {
        if (CCList.get(i).getValue('a_Market_Designation').getSimpleValue().indexOf("SA") >= 0) {
...
if(sortOrderArray.length != 0){
	...
	if(!isCCSortOrderPresent){
		...
		node.getValue('a_error_message').setSimpleValue('There is no CC with Valid Market Designation Value i.e., SA in this case.');
		return 'There is no CC with Valid Market Designation Value i.e., SA in this case.';
