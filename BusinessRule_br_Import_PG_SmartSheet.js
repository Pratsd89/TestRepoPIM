...
{
  "id" : "br_Import_PG_SmartSheet",
  "type" : "BusinessAction",
  "setupGroups" : [ "Super PDP Consolidation" ],
  "name" : "Create Product Group Using SmartSheet",
  "description" : null,
  "scope" : "Global",
...
if (primaryStyleNumImp != null && primaryStyleNumImp.trim() !== '' && !primaryStyleNumImp.equalsIgnoreCase('DELETE')) {
    ...
    var duplicateStyleGroup = pgLibrary.getDuplicateStyleGroup(node, refType);
    var superPDPMkt = null;

    if (duplicateStyleGroup != null) {
        superPDPMkt = duplicateStyleGroup.getValue("a_SuperPDP_Market").getSimpleValue();
    }
    var canadaManager = "";
    step.executeInContext("EN_CA", function (contextManager) {
        canadaManager = contextManager;
    ...
    if (duplicateStyleGroup != null) {
        var dsgPrimaryProductNum = duplicateStyleGroup.getValue("a_Primary_Product").getSimpleValue();
        var dsgPrimaryStyleID = pgLibrary.getPrimaryStyleID(dsgPrimaryProductNum);
        var dsgPrimaryStyle = step.getProductHome().getProductByID(dsgPrimaryStyleID);
        var dsgRefs = duplicateStyleGroup.getReferences(refType).toArray();
        if (node.getID() == dsgPrimaryStyleID) {
            if (context == "EN_US" && superPDPMkt.contains("US") && superPDPMkt.contains("CAN") && CanadaDSG != null) {
                //Clear US refs
                pgLibrary.clearRefs(dsgRefs, step, MVGRefType);

                //Clear CAN refs
                var dsgCANRefs = CanadaDSG.getReferences(refType).toArray();
                pgLibrary.clearRefs(dsgCANRefs, step, MVGRefType);

                //Delete group
                //duplicateStyleGroup.delete();
            }
            else if ((context == "EN_US" && superPDPMkt == "US") || (context == "EN_CA" && superPDPMkt == "CAN")) {
                pgLibrary.clearRefs(dsgRefs, step, MVGRefType);
                duplicateStyleGroup.getValue("a_SuperPDP_Market").setSimpleValue(null);
                duplicateStyleGroup.getValue("a_Product_Grouping_Start_date").setSimpleValue(null);
                duplicateStyleGroup.getValue("a_Product_Grouping_End_Date").setSimpleValue(null);
                //duplicateStyleGroup.delete();
            }
            else if (context == "EN_CA" && superPDPMkt != null && superPDPMkt.contains("US") && superPDPMkt.contains("CAN")) {
                throw ("It is not permitted to delete a Group in EN_CA when inheritance applies.");
            }
        }
        else {
            if (context == "EN_US" && superPDPMkt.contains("US") && superPDPMkt.contains("CAN") && CanadaDSG != null) {
                ...
                for (var i = 0; i < dsgRefs.length; i++) {
                    ...
                    if (currentStyle.getID() == node.getID()) {
                        pgLibrary.clearStyleData(currentStyle);
                        dsgRefs[i].delete();
                        pgLibrary.removeFromMVGIfPresent(currentStyle, step, MVGRefType);
                        dsgPrimaryStyle.getValue("a_main_last_modified_date").setSimpleValue(iso.format(time));
                        break;
                    }
                }
                //Clear CAN refs
                var dsgCANRefs = CanadaDSG.getReferences(refType).toArray();
                for (var i = 0; i < dsgCANRefs.length; i++) {
                    var currentStyle = dsgCANRefs[i].getTarget();
                    if (currentStyle.getID() == node.getID()) {
                        pgLibrary.clearStyleData(currentStyle);
                        dsgCANRefs[i].delete();
                        pgLibrary.removeFromMVGIfPresent(currentStyle, step, MVGRefType);
                ...
                var styleNum = node.getvalue("a_Style_Number").getSimpleValue();
                throw ("You cannot remove style " + styleNum + " from the Product Group as you're not in the right context");
            }
        }
        duplicateStyleGroup.getValue("a_main_last_modified_date").setSimpleValue(iso.format(time));
    }
    else if (duplicateStyleGroup == null) {
        pgLibrary.clearStyleData(node);
    }
}
