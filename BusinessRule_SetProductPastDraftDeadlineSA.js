...
{
  "id" : "SetProductPastDraftDeadlineSA",
  "type" : "BusinessAction",
  "setupGroups" : [ "Actions" ],
  "name" : "SetProductPastDraftDeadlineSA",
  "description" : null,
  "scope" : "Global",
  "validObjectTypes" : [ ],
...
deadlineTime = node.getTaskByID("wf_NewStyleEnrichmentSA", "NewStyleEnrichState1").getDeadline().getTime();
...
if(parseInt(deadlineTime) < parseInt(currTime)){
	...
	deadlineTime = node.getTaskByID("wf_CCEnrichmentSA", "NewCCEnrichState1").getDeadline().getTime();
	...
	if(parseInt(deadlineTime) < parseInt(currTime)){
		...
		deadlineTime = node.getTaskByID("wf_NewSKUEnrichmentSA", "NewSKUEnrich1").getDeadline().getTime();
