    ...
    step.executeInContext(context, function (contextManager) {
        ...
        if (objType == "Style") {
            ...
            if (ctxLifeCycleStatus == "Approved" || (ctxPhotoStatus != null && ctxPhotoStatus.contains("Complete"))) {
                result = "Approved";
            } else if (ctxPhotoStatus == "Ready For Review") {
                result = "RFR";
            } else if (ctxPhotoStatus == "Shot Request Submitted") {
                result = "Shots";
            }
        }
    });
...
function deleteExistingShotRef(saNode) {
    if (currentContext == "EN_SA") {
        var ccLcs = saNode.getValue("a_CC_Life_Cycle_Status").getSimpleValue();
        if (ccLcs != null && ccLcs != "Approved") {
            var ccNum = saNode.getValue("a_CC_Number").getSimpleValue();
            //Delete Shot links
            saNode.queryReferences(shotRef).forEach(function (refInstance) {
            ...
            for (var j = 0; j < 14; j++) {
                ...
                refImg.addAll(saNode.getReferences(imgRef[j]));
...
if (currentContext != "FR_CA" && currentContext != "JA_JP") {
    ...
    if (objectType == "CustomerChoice") {
        ...
        if (aclMarket == currentMarket) {
            if (currentMarket == "SA") {
                if (checkStatus(node, "EN_SA") != "Approved") {
                    var flag = false;
                    var BOMUSNode = checkBOMRef(node);

                    function tryCopy(fromNode, context) {
                        deleteExistingShotRef(node);
                        CopyShots(fromNode, context);
                        if (fromNode == node) {
                            deleteBOMRefIfExists(node);
                        }
                    }

                    var priorities = ["Approved", "RFR", "Shots"];
                    var sources = [
                        { cc: BOMUSNode, context: "EN_US", market: "BOM" },
                        { cc: node, context: "EN_US", market: "US" },
                        { cc: node, context: "EN_JP", market: "JPN" }
                    ];

                    for (var i = 0; i < priorities.length; i++) {
                        var statusNeeded = priorities[i];
                        for (var j = 0; j < sources.length; j++) {
                            var source = sources[j];
                            if (!source.cc) continue;

                            // BOM doesn't require market check; US and JPN do
                            if (source.market != "BOM" && (!mktDsg || !mktDsg.contains(source.market))) continue;

                            var status = checkStatus(source.cc, source.context);
                            if (status == statusNeeded) {
                                tryCopy(source.cc, source.context);
                                flag = true;
                                break;
                            }
                        }
